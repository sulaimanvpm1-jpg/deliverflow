...
const Badge = ({ status }) => {
  ...
  return <span style={{ background:c.bg, color:c.color, borderRadius:20, padding:"3px 10px", fontSize:11, fontWeight:600, fontFamily:"var(--font-body)", whiteSpace:"nowrap" }}>{c.icon} {c.label}</span>;
};
const Pill = ({ label, active, onClick, count }) => (
  <button onClick={onClick} style={{ background:active?"#00D4FF":"rgba(255,255,255,.07)", color:active?"#0A0F1E":"rgba(255,255,255,.6)", border:"none", borderRadius:20, padding:"6px 14px", fontSize:12, fontFamily:"var(--font-body)", fontWeight:active?600:400, cursor:"pointer", whiteSpace:"nowrap", display:"flex", alignItems:"center", gap:5 }}>
...
const Toast = ({ msg, toastKind="info" }) => {
  ...
  return <div style={{ position:"fixed", top:16, left:"50%", transform:"translateX(-50%)", background:c2, borderRadius:30, padding:"10px 20px", fontFamily:"var(--font-body)", color:"#fff", fontSize:13, zIndex:400, whiteSpace:"nowrap", boxShadow:"0 8px 32px rgba(0,0,0,.4)", animation:"fadeIn .3s" }}>{msg}</div>;
};
...
setProgress("Found " + result.orders.length + " orders for " + (result.driverName || "driver") + "!");
  ...
  return (
    <div style={{ background:"rgba(255,255,255,.04)", border:"1.5px dashed rgba(0,212,255,.35)", borderRadius:18, padding:24 }}>
      <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:16, fontWeight:700, marginBottom:4 }}> Upload Delivery PDF</div>
      <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:13, marginBottom:18 }}>SAP Business One   Bill-wise Details report</div>
      ...
      <div onClick={() => !parsing && fileRef.current.click()}
        ...
        {parsing ? (
          <div>
            ...
            <div style={{ fontFamily:"var(--font-body)", color:"#00D4FF", fontSize:14 }}>{progress}</div>
          </div>
        ) : file ? (
          <div>
            <div style={{ fontSize:32, marginBottom:8 }}></div>
            <div style={{ fontFamily:"var(--font-head)", color:"#10B981", fontSize:14, fontWeight:700 }}>{file.name}</div>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12, marginTop:4 }}>{progress || "Click to upload a different file"}</div>
          </div>
        ) : (
          <div>
            <div style={{ fontSize:40, marginBottom:10 }}></div>
            <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:15, fontWeight:600 }}>Drop PDF here or click to browse</div>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:13, marginTop:6 }}>Supports SAP Business One delivery reports</div>
          </div>
        )}
      </div>

      {error && <div style={{ background:"rgba(245,158,11,.1)", border:"1px solid rgba(245,158,11,.3)", borderRadius:10, padding:"10px 12px", marginTop:10, fontFamily:"var(--font-body)", color:"#F59E0B", fontSize:13 }}> {error}</div>}
  ...
  if (!show) return (
    <div style={{ display:"flex", gap:8, marginBottom:12 }}>
      <button onClick={function(){ setShow(true); }}
        style={{ flex:1, background:"rgba(255,107,53,.08)", border:"1px dashed rgba(255,107,53,.4)", borderRadius:12, padding:"12px", color:"#FF6B35", fontFamily:"var(--font-head)", fontSize:13, fontWeight:700, cursor:"pointer" }}>
        + Add Manual Order
      </button>
      <LabelScanner
  ...
  return (
    <div style={{ background:"rgba(255,255,255,.04)", border:"1px solid rgba(255,107,53,.3)", borderRadius:14, padding:16, marginBottom:14 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
        <div style={{ fontFamily:"var(--font-head)", color:"#FF6B35", fontSize:14, fontWeight:800 }}>Manual Order Entry</div>
      ...
      {scanMsg && (
        <div style={{ background: scanMsg.startsWith("✅") ? "rgba(16,185,129,.1)" : "rgba(239,68,68,.1)", border:"1px solid " + (scanMsg.startsWith("✅") ? "rgba(16,185,129,.3)" : "rgba(239,68,68,.3)"), borderRadius:10, padding:"8px 12px", fontFamily:"var(--font-body)", fontSize:12, color: scanMsg.startsWith("✅") ? "#10B981" : "#EF4444", marginBottom:12 }}>
          {scanMsg}
        </div>
      )}

      {/* Store */}
      <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11, marginBottom:6 }}>STORE</div>
      <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:8 }}>
        {["Trikart Online","Webstore Online","ReStore Online","Other"].map(function(s){
          return <button key={s} onClick={function(){ setStore(s); if(s!=="Other") setCustomStoreName(""); }}
            style={{ background:store===s?"rgba(0,212,255,.15)":"rgba(255,255,255,.06)", border:store===s?"1.5px solid #00D4FF":"1px solid rgba(255,255,255,.1)", borderRadius:20, padding:"5px 12px", color:store===s?"#00D4FF":"rgba(255,255,255,.6)", fontFamily:"var(--font-body)", fontSize:12, cursor:"pointer" }}>{s}</button>;
        })}
      </div>
      {store === "Other" && (
        <div style={{ marginBottom:12 }}>
          <input type="text" value={customStoreName} onChange={function(e){ setCustomStoreName(e.target.value); }}
            ...
            style={{ width:"100%", boxSizing:"border-box", background:"rgba(255,255,255,.07)", border:"1px solid rgba(0,212,255,.3)", borderRadius:10, padding:"9px 12px", color:"#00D4FF", fontFamily:"var(--font-body)", fontSize:13, outline:"none" }} />
        </div>
      )}

      {/* Driver */}
      <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11, marginBottom:6 }}>ASSIGN TO DRIVER</div>
      <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:12 }}>
        {driverList.map(function(d){
          return <button key={d.id} onClick={function(){ setDriver(d.id); }}
            style={{ background:driver===d.id?"rgba(0,212,255,.15)":"rgba(255,255,255,.06)", border:driver===d.id?"1.5px solid #00D4FF":"1px solid rgba(255,255,255,.1)", borderRadius:20, padding:"5px 12px", color:driver===d.id?"#00D4FF":"rgba(255,255,255,.6)", fontFamily:"var(--font-body)", fontSize:12, cursor:"pointer" }}>{d.name}</button>;
        })}
      </div>
      ...
      {[
        ...
        return (
          <div key={f[0]} style={{ marginBottom:10 }}>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11, marginBottom:4 }}>{f[0].toUpperCase()}</div>
            <input type={f[3]} value={f[1]} onChange={function(e){ f[2](e.target.value); }} placeholder={f[4]}
              style={{ width:"100%", boxSizing:"border-box", background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,255,255,.12)", borderRadius:10, padding:"10px 12px", color:"#fff", fontFamily:"var(--font-body)", fontSize:13, outline:"none" }} />
          </div>
        );
      })}
      ...
      <div style={{ marginBottom:10 }}>
        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11, marginBottom:4 }}>AREA (KUWAIT)</div>
        <input type="text" value={areaSearch}
          onChange={function(e){ setAreaSearch(e.target.value); setArea(""); }}
          placeholder="Type to search area..."
          style={{ width:"100%", boxSizing:"border-box", background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,255,255,.12)", borderRadius:area?"10px 10px 0 0":"10", padding:"10px 12px", color:"#fff", fontFamily:"var(--font-body)", fontSize:13, outline:"none", borderBottom:areaSearch&&!area?"none":"1px solid rgba(255,255,255,.12)" }} />
        {area ? (
          <div style={{ background:"rgba(0,212,255,.1)", border:"1px solid rgba(0,212,255,.3)", borderRadius:10, padding:"8px 12px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <span style={{ fontFamily:"var(--font-body)", color:"#00D4FF", fontSize:13, fontWeight:600 }}>{area}</span>
          ...
          <div style={{ background:"#1A2035", border:"1px solid rgba(255,255,255,.15)", borderRadius:"0 0 10px 10px", maxHeight:160, overflowY:"auto" }}>
            {ALL_AREAS.filter(function(a){ return a.toLowerCase().includes(areaSearch.toLowerCase()); }).slice(0,20).map(function(a){
              return <div key={a} onClick={function(){ setArea(a); setAreaSearch(a); }}
                style={{ padding:"8px 12px", fontFamily:"var(--font-body)", color:"rgba(255,255,255,.8)", fontSize:12, cursor:"pointer", borderBottom:"1px solid rgba(255,255,255,.05)" }}>{a}</div>;
            })}
          </div>
        ) : null}
      </div>
      ...
      {/* Payment */}
      <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11, marginBottom:8 }}>PAYMENT TYPE</div>
      <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:14 }}>
        {MANUAL_PAYS.map(function(p){
          return <button key={p} onClick={function(){ setPay(p); }}
            style={{ background:pay===p?"rgba(16,185,129,.15)":"rgba(255,255,255,.06)", border:pay===p?"1.5px solid #10B981":"1px solid rgba(255,255,255,.1)", borderRadius:20, padding:"5px 12px", color:pay===p?"#10B981":"rgba(255,255,255,.6)", fontFamily:"var(--font-body)", fontSize:12, cursor:"pointer" }}>{p}</button>;
        })}
      </div>

      {err && <div style={{ color:"#EF4444", fontFamily:"var(--font-body)", fontSize:12, marginBottom:8 }}>{err}</div>}

      <button onClick={submit}
        style={{ width:"100%", background:"linear-gradient(135deg,#FF6B35,#FF3D71)", border:"none", borderRadius:12, padding:"13px", color:"#fff", fontFamily:"var(--font-head)", fontSize:14, fontWeight:700, cursor:"pointer" }}>
        Add Order
      </button>
    </div>
  );
      ...
      {!parsed ? (
        <>
          ...
          {allOrders.length > 0 && (
            <div style={{ marginTop:20 }}>
              <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:15, fontWeight:700, marginBottom:12 }}>Today&apos;s Assigned Orders</div>
                ...
                return (
                  <div key={d.id} style={{ background:"rgba(255,255,255,.04)", border:"1px solid rgba(255,255,255,.08)", borderRadius:14, padding:14, marginBottom:10 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
                      <div style={{ width:38, height:38, borderRadius:"50%", background:"linear-gradient(135deg,#FF6B35,#FF3D71)", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"var(--font-head)", fontWeight:800, color:"#fff", fontSize:13 }}>{d.avatar}</div>
                      <div style={{ flex:1 }}>
                        <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:14, fontWeight:700 }}>{d.name}</div>
                        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12 }}>{dOrders.length} orders</div>
                      </div>
                      <div style={{ textAlign:"right" }}>
                        <div style={{ fontFamily:"var(--font-head)", color:"#10B981", fontSize:14, fontWeight:700 }}>{done}/{dOrders.length}</div>
                        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11 }}>delivered</div>
                      </div>
                    </div>
                    <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:6 }}>
                      <div style={{ background:"rgba(0,212,255,.12)", borderRadius:10, padding:"6px 4px", textAlign:"center" }}>
                        <div style={{ fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:14, fontWeight:800 }}>{collected}</div>
                        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:9 }}>Collected</div>
                      </div>
                      <div style={{ background:"rgba(16,185,129,.12)", borderRadius:10, padding:"6px 4px", textAlign:"center" }}>
                        <div style={{ fontFamily:"var(--font-head)", color:"#10B981", fontSize:14, fontWeight:800 }}>{done}</div>
                        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:9 }}>Delivered</div>
                      </div>
                      <div style={{ background:"rgba(239,68,68,.1)", borderRadius:10, padding:"6px 4px", textAlign:"center" }}>
                        <div style={{ fontFamily:"var(--font-head)", color:"#EF4444", fontSize:14, fontWeight:800 }}>{cancelled}</div>
                        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:9 }}>Cancelled</div>
                      </div>
                      <div style={{ background:"rgba(139,92,246,.12)", borderRadius:10, padding:"6px 4px", textAlign:"center" }}>
                        <div style={{ fontFamily:"var(--font-head)", color:"#8B5CF6", fontSize:14, fontWeight:800 }}>{postponed}</div>
                        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:9 }}>Postponed</div>
                      </div>
                    </div>
                  </div>
                );
        ...
        <div>
          <div style={{ background:"rgba(0,212,255,.08)", border:"1px solid rgba(0,212,255,.3)", borderRadius:16, padding:16, marginBottom:16 }}>
            <div style={{ fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:15, fontWeight:700 }}>Orders Extracted: {parsed.orders.length}</div>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:13, marginTop:2 }}>
              {parsed.company} &nbsp; Suggested: <strong style={{ color:"#fff" }}>{parsed.driverName}</strong>
            </div>
            <div style={{ display:"flex", gap:12, marginTop:10 }}>
              <div style={{ fontFamily:"var(--font-body)", color:"#10B981", fontSize:12 }}>New: {parsed.fresh ? parsed.fresh.length : parsed.orders.length}</div>
              {parsed.dupes && parsed.dupes.length > 0 && (
                <div style={{ fontFamily:"var(--font-body)", color:"#F59E0B", fontSize:12, fontWeight:600 }}>Duplicates: {parsed.dupes.length}</div>
              )}
            </div>
          </div>
          ...
          {parsed.dupes && parsed.dupes.length > 0 && (
            <div style={{ background:"rgba(245,158,11,.08)", border:"1px solid rgba(245,158,11,.3)", borderRadius:14, padding:"12px 14px", marginBottom:14 }}>
              <div style={{ fontFamily:"var(--font-head)", color:"#F59E0B", fontSize:13, fontWeight:700, marginBottom:6 }}>
                {parsed.dupes.length} Duplicate Order{parsed.dupes.length > 1 ? "s" : ""} Found
              </div>
              <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:12, marginBottom:10, lineHeight:1.5 }}>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                {parsed.dupes.map(function(o) {
                  return (
                    <span key={o.invoiceNo} style={{ background:"rgba(245,158,11,.15)", border:"1px solid rgba(245,158,11,.3)", borderRadius:20, padding:"3px 10px", fontFamily:"var(--font-body)", color:"#F59E0B", fontSize:11 }}>
          ...
          {["ReStore Online","Trikart Online","Webstore Online"].map(function(s) {
            ...
            return (
              <div key={s} style={{ background:"rgba(255,255,255,.03)", borderRadius:10, padding:"8px 12px", marginBottom:6, display:"flex", justifyContent:"space-between" }}>
                <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.7)", fontSize:13 }}>{s}</span>
                <span style={{ fontFamily:"var(--font-head)", color:"#FF6B35", fontSize:13, fontWeight:700 }}>{so.length} orders</span>
              </div>
            );
          })}

          <div style={{ marginTop:16, marginBottom:12 }}>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:12, marginBottom:8 }}>ASSIGN TO DRIVER</div>
              ...
              {DRIVERS.map(function(d) {
                return (
                  <button key={d.id} onClick={function() { setAssignTo(d.id); }} style={{ display:"flex", alignItems:"center", gap:8, background:assignTo===d.id?"rgba(0,212,255,.15)":"rgba(255,255,255,.05)", border:assignTo===d.id?"1.5px solid #00D4FF":"1px solid rgba(255,255,255,.1)", borderRadius:12, padding:"10px 14px", cursor:"pointer" }}>
                    <div style={{ width:30, height:30, borderRadius:"50%", background:"linear-gradient(135deg,#FF6B35,#FF3D71)", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"var(--font-head)", fontSize:11, fontWeight:800, color:"#fff" }}>{d.avatar}</div>
                    <span style={{ fontFamily:"var(--font-head)", color:assignTo===d.id?"#00D4FF":"#fff", fontSize:13, fontWeight:600 }}>{d.name}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:12 }}>{selected.size} of {parsed.orders.length} selected</div>
            ...
            }} style={{ background:"none", border:"none", color:"#00D4FF", fontFamily:"var(--font-body)", fontSize:12, cursor:"pointer" }}>
              ...
              return (
                <div key={i} onClick={function() { toggleSelect(i); }}
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontFamily:"var(--font-body)", color:"#fff", fontSize:13, fontWeight:500 }}>{o.customer}</div>
                    <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11 }}>#{o.invoiceNo} · {o.store}</div>
                    {o.onlineOrderNo ? <div style={{ fontFamily:"var(--font-body)", color:"#00D4FF", fontSize:10, fontWeight:700 }}>Online Order: {o.onlineOrderNo}</div> : null}
                  </div>
                  <div style={{ textAlign:"right", flexShrink:0 }}>
                    <div style={{ fontFamily:"var(--font-head)", color:"#FF6B35", fontSize:13, fontWeight:700 }}>{fmt(o.total)}</div>
                    <div style={{ fontFamily:"var(--font-body)", fontSize:10 }}><PaymentBadge payType={o.paymentType} small /></div>
                  </div>
                </div>
              );
            })}
          ...
          <div style={{ display:"flex", gap:10 }}>
            <button onClick={function() { setParsed(null); setSelected(new Set()); }} style={{ flex:1, background:"rgba(255,255,255,.07)", border:"none", borderRadius:12, padding:14, color:"#fff", fontFamily:"var(--font-body)", cursor:"pointer" }}>Back</button>
            <button onClick={handleAssign} disabled={!assignTo || selected.size===0}
              style={{ flex:2, background:assignTo&&selected.size>0?"linear-gradient(135deg,#00D4FF,#7C3AED)":"rgba(255,255,255,.1)", border:"none", borderRadius:12, padding:14, color:"#fff", fontFamily:"var(--font-head)", fontWeight:700, fontSize:14, cursor:assignTo?"pointer":"default" }}>
  ...
  return (
    <div style={{ flex:1, overflowY:"auto", padding:"0 16px 80px" }}>
      <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍 Search customer, invoice, order no..."
        style={{ width:"100%", background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,255,255,.1)", borderRadius:12, padding:"11px 16px", color:"#fff", fontFamily:"var(--font-body)", fontSize:14, marginBottom:12, boxSizing:"border-box", outline:"none" }} />
      ...
      <div style={{ background:"rgba(255,107,53,.08)", border:"1px solid rgba(255,107,53,.2)", borderRadius:12, padding:"10px 14px", marginBottom:10, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:13 }}>Showing {filtered.length} orders</span>
        <span style={{ fontFamily:"var(--font-head)", color:"#FF6B35", fontSize:14, fontWeight:700 }}>{fmt(totalKD)}</span>
      </div>
      ...
      <div style={{ display:"flex", justifyContent:"flex-end", marginBottom:10, gap:6 }}>
        {!selectMode ? (
          <button onClick={function(){ setSelectMode(true); setSelected(new Set()); }}
            style={{ background:"rgba(239,68,68,.1)", border:"1px solid rgba(239,68,68,.25)", borderRadius:8, padding:"4px 14px", color:"#EF4444", fontFamily:"var(--font-body)", fontSize:12, cursor:"pointer" }}>
            Select to Remove
          </button>
        ) : (
          <>
            <button onClick={function(){ setSelected(new Set(filtered.map(function(o){ return o.id||o.invoiceNo; }))); }}
              style={{ background:"rgba(255,255,255,.07)", border:"none", borderRadius:8, padding:"4px 10px", color:"rgba(255,255,255,.6)", fontFamily:"var(--font-body)", fontSize:12, cursor:"pointer" }}>
              Select All
            </button>
            {selected.size > 0 && (
              <button onClick={removeSelected}
                style={{ background:"rgba(239,68,68,.2)", border:"1px solid rgba(239,68,68,.4)", borderRadius:8, padding:"4px 12px", color:"#EF4444", fontFamily:"var(--font-body)", fontSize:12, fontWeight:600, cursor:"pointer" }}>
                Remove ({selected.size})
              </button>
            )}
            <button onClick={function(){ setSelectMode(false); setSelected(new Set()); }}
              style={{ background:"none", border:"1px solid rgba(255,255,255,.1)", borderRadius:8, padding:"4px 10px", color:"rgba(255,255,255,.4)", fontFamily:"var(--font-body)", fontSize:12, cursor:"pointer" }}>
              Cancel
            </button>
          </>
        )}
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign:"center", color:"rgba(255,255,255,.3)", fontFamily:"var(--font-body)", padding:40 }}>No orders found</div>
    ...
    <div style={{ background:"rgba(255,255,255,.04)", border:"1px solid rgba(255,255,255,.07)", borderRadius:14, marginBottom:8, overflow:"hidden" }}>
      <div onClick={() => setExp(e => !e)} style={{ padding:"12px 14px", cursor:"pointer" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:4 }}>
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:14, fontWeight:700 }}>{order.customer}</div>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:11, marginTop:1 }}>#{order.invoiceNo} {order.onlineOrderNo ? "  OO:" + (order.onlineOrderNo) : ""}   {order.store}</div>
          </div>
          <Badge status={order.status} />
        </div>
        <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
          <span style={{ fontFamily:"var(--font-head)", color:"#FF6B35", fontSize:13, fontWeight:700 }}>{fmt(order.total)}</span>
          <span style={{ fontFamily:"var(--font-body)", fontSize:11 }}><PaymentBadge payType={order.paymentType} small /></span>
          {driver && <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11 }}> {driver.name}</span>}
          {order.scanned && <span style={{ fontFamily:"var(--font-body)", color:"#00D4FF", fontSize:11 }}> Collected</span>}
        </div>
      </div>
      {exp && (
        <div style={{ borderTop:"1px solid rgba(255,255,255,.06)", padding:"12px 14px", background:"rgba(0,0,0,.15)" }}>
          <div style={{ display:"flex", alignItems:"center", flexWrap:"wrap", gap:4, marginBottom:4  }}>{(function(){ var _a = detectArea(order.address); return _a ? <span style={{ background:"rgba(0,212,255,.12)", border:"1px solid rgba(0,212,255,.25)", borderRadius:20, padding:"1px 9px", fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:10, fontWeight:700, marginRight:6 }}>{_a}</span> : null; })()}<span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.55)", fontSize:12  }}>{order.address}</span></div>
          <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.55)", fontSize:12, marginBottom:4 }}> {order.phone}</div>
          {order.note && <div style={{ fontFamily:"var(--font-body)", color:"#C4B5FD", fontSize:12 }}> {order.note}</div>}
        </div>
      )}
    </div>
  );
      ...
      <div style={{ width:"100%", background:"#0F1629", borderRadius:"24px 24px 0 0", padding:24 }}>
        {sent ? (
          <div style={{ textAlign:"center", padding:"20px 0" }}>
            ...
            <div style={{ fontFamily:"var(--font-head)", color:"#10B981", fontSize:18, fontWeight:700 }}>Transfer Requested!</div>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:13, marginTop:8 }}>
              Admin has been notified. Awaiting approval.
            </div>
            <button onClick={onClose} style={{ marginTop:20, background:"rgba(255,255,255,.08)", border:"none", borderRadius:12, padding:"12px 32px", color:"#fff", fontFamily:"var(--font-body)", cursor:"pointer" }}>Close</button>
          </div>
        ) : (
          <>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
              <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:17, fontWeight:700 }}> Transfer Order</div>
              <button onClick={onClose} style={{ background:"rgba(255,255,255,.08)", border:"none", borderRadius:20, padding:"5px 12px", color:"#fff", fontFamily:"var(--font-body)", fontSize:12, cursor:"pointer" }}>x</button>
            </div>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12, marginBottom:16 }}>
            ...
            <div style={{ background:"rgba(255,107,53,.07)", border:"1px solid rgba(255,107,53,.2)", borderRadius:14, padding:"12px 14px", marginBottom:16 }}>
              <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:14, fontWeight:700 }}>{order.customer}</div>
              <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12, marginTop:2 }}>#{order.invoiceNo}   {order.store}   {fmt(order.total)}</div>
            </div>

            {/* Transfer type */}
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.45)", fontSize:12, marginBottom:8 }}>TRANSFER TO</div>
            <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:14 }}>
              ...
              <button onClick={() => setToDriver(fromDriverId)} style={{ display:"flex", alignItems:"center", gap:10, background:toDriver===fromDriverId?"rgba(0,212,255,.12)":"rgba(255,255,255,.05)", border:toDriver===fromDriverId?"1.5px solid #00D4FF":"1px solid rgba(255,255,255,.1)", borderRadius:12, padding:"11px 14px", cursor:"pointer" }}>
                <div style={{ width:32, height:32, borderRadius:"50%", background:"linear-gradient(135deg,#00D4FF,#7C3AED)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:800, color:"#fff", fontFamily:"var(--font-head)" }}>ME</div>
                <div style={{ textAlign:"left" }}>
                  <div style={{ fontFamily:"var(--font-head)", color:toDriver===fromDriverId?"#00D4FF":"#fff", fontSize:13, fontWeight:600 }}>Take to me ({DRIVERS.find(d=>d.id===fromDriverId)?.name})</div>
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:11 }}>Request to take this order from {ownerDriver?.name}</div>
                </div>
              </button>
              {/* Other drivers */}
              {otherDrivers.filter(d => d.id !== order.driverId).map(d => (
                <button key={d.id} onClick={() => setToDriver(d.id)} style={{ display:"flex", alignItems:"center", gap:10, background:toDriver===d.id?"rgba(139,92,246,.12)":"rgba(255,255,255,.05)", border:toDriver===d.id?"1.5px solid #8B5CF6":"1px solid rgba(255,255,255,.1)", borderRadius:12, padding:"11px 14px", cursor:"pointer" }}>
                  <div style={{ width:32, height:32, borderRadius:"50%", background:"linear-gradient(135deg,#FF6B35,#FF3D71)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:800, color:"#fff", fontFamily:"var(--font-head)" }}>{d.avatar}</div>
                  <div style={{ textAlign:"left" }}>
                    <div style={{ fontFamily:"var(--font-head)", color:toDriver===d.id?"#8B5CF6":"#fff", fontSize:13, fontWeight:600 }}>Transfer to {d.name}</div>
                    <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:11 }}>Send to {d.name}&apos;s order list</div>
                  </div>
                </button>
              ))}
            </div>
            ...
            <textarea value={reason} onChange={e => setReason(e.target.value)} placeholder="Reason for transfer (optional)..."
              style={{ width:"100%", background:"rgba(255,255,255,.05)", border:"1px solid rgba(255,255,255,.1)", borderRadius:12, padding:12, color:"#fff", fontFamily:"var(--font-body)", fontSize:13, resize:"none", height:60, boxSizing:"border-box", outline:"none", marginBottom:14 }} />

            <div style={{ display:"flex", gap:10 }}>
              <button onClick={onClose} style={{ flex:1, background:"rgba(255,255,255,.07)", border:"none", borderRadius:12, padding:13, color:"#fff", fontFamily:"var(--font-body)", cursor:"pointer" }}>Cancel</button>
              <button onClick={submit} disabled={!toDriver} style={{ flex:2, background:toDriver?"linear-gradient(135deg,#FF6B35,#FF3D71)":"rgba(255,255,255,.1)", border:"none", borderRadius:12, padding:13, color:"#fff", fontFamily:"var(--font-head)", fontWeight:700, cursor:toDriver?"pointer":"default" }}>
    ...
    <div style={{ flex:1, overflowY:"auto", padding:"0 16px 80px" }}>
      ...
      <div style={{ background:"rgba(255,255,255,.05)", borderRadius:16, padding:16, marginBottom:14 }}>
        <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
          <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:13 }}>Collection Progress</span>
          <span style={{ fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:14, fontWeight:700 }}>{scanned.length}/{myOrders.length}</span>
        </div>
        ...
        <div style={{ display:"flex", gap:12, marginTop:10 }}>
          <span style={{ fontFamily:"var(--font-body)", color:"#00D4FF", fontSize:12 }}> {scanned.length} collected</span>
          <span style={{ fontFamily:"var(--font-body)", color:"#F59E0B", fontSize:12 }}> {unscanned.length} remaining</span>
        </div>
      </div>
        ...
        <>
            ...
            <button onClick={function() {
              ...
              {scanning ? (
                <div style={{ textAlign:"center" }}>
                  ...
                  <div style={{ color:"rgba(255,255,255,.5)", fontFamily:"var(--font-body)", fontSize:13, marginTop:12 }}>Opening camera...</div>
                </div>
              ) : (
                <>
                  <div style={{ fontSize:36, marginBottom:8 }}>[ ]</div>
                  <div style={{ fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:14, fontWeight:700 }}>Tap to Scan Barcode</div>
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11, marginTop:4 }}>Opens phone camera</div>
                </>
              )}
              {["tl","tr","bl","br"].map(p => (
          ...
          <div style={{ display:"flex", gap:10, marginBottom:10 }}>
            <input id="scan-manual-input" value={scanInput}
              ...
              style={{ flex:1, background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,255,255,.1)", borderRadius:12, padding:"11px 14px", color:"#fff", fontFamily:"var(--font-body)", fontSize:14, outline:"none" }} />
            <button onClick={function() { doScan(scanInput); }} style={{ background:"#00D4FF", border:"none", borderRadius:12, padding:"0 16px", color:"#0A0F1E", fontFamily:"var(--font-head)", fontWeight:700, cursor:"pointer" }}>Go</button>
          </div>
          <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.25)", fontSize:11, marginBottom:14, textAlign:"center" }}>
          ...
          {scanError && (
            <div style={{ background:"rgba(239,68,68,.1)", border:"1px solid rgba(239,68,68,.3)", borderRadius:12, padding:"12px 14px", marginBottom:12, display:"flex", alignItems:"center", gap:10 }}>
              ...
              <span style={{ fontFamily:"var(--font-body)", color:"#EF4444", fontSize:13 }}>{scanError}</span>
            </div>
          )}
          ...
          {wrongDriver && (
            <div style={{ background:"rgba(239,68,68,.08)", border:"1.5px solid rgba(239,68,68,.4)", borderRadius:16, padding:16, marginBottom:12 }}>
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
                ...
                <div>
                  <div style={{ fontFamily:"var(--font-head)", color:"#EF4444", fontSize:14, fontWeight:700 }}>Not in your order list!</div>
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12 }}>
              ...
              <div style={{ background:"rgba(0,0,0,.2)", borderRadius:10, padding:"10px 12px", marginBottom:12 }}>
                <div style={{ fontFamily:"var(--font-body)", color:"#fff", fontSize:13, fontWeight:500 }}>{wrongDriver.customer}</div>
                <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11, marginTop:2 }}>#{wrongDriver.invoiceNo}   {wrongDriver.store}   {fmt(wrongDriver.total)}</div>
              </div>
              <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.45)", fontSize:12, marginBottom:8 }}>Do you want to request a transfer?</div>
              <div style={{ display:"flex", gap:8 }}>
                <button onClick={() => { setWrongDriver(null); setScanInput(""); }}
                  style={{ flex:1, background:"rgba(255,255,255,.07)", border:"none", borderRadius:10, padding:"10px", color:"#fff", fontFamily:"var(--font-body)", fontSize:13, cursor:"pointer" }}>
                  Dismiss
                </button>
                <button onClick={() => { onOpenTransfer(wrongDriver); setWrongDriver(null); setScanInput(""); setScanError(""); }}
                  style={{ flex:2, background:"linear-gradient(135deg,#FF6B35,#FF3D71)", border:"none", borderRadius:10, padding:"10px", color:"#fff", fontFamily:"var(--font-head)", fontWeight:700, fontSize:13, cursor:"pointer" }}>
                   Request Transfer
                </button>
              </div>
            </div>
          ...
          {scanResult && (
            <div style={{ background:"rgba(0,212,255,.08)", border:"1px solid rgba(0,212,255,.35)", borderRadius:16, padding:16, marginBottom:10 }}>
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
                ...
                <div style={{ fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:14, fontWeight:700 }}>Your Order - Ready to Collect</div>
              </div>
              <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:15, fontWeight:700 }}>{scanResult.customer}</div>
              <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:13, marginTop:2 }}>Invoice #{scanResult.invoiceNo}{scanResult.onlineOrderNo ? "   OO: " + (scanResult.onlineOrderNo) : ""}</div>
              <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:13 }}> {scanResult.store}</div>
              <div style={{ display:"flex", gap:10, marginTop:8, alignItems:"center" }}>
                <span style={{ fontFamily:"var(--font-head)", color:"#FF6B35", fontSize:16, fontWeight:800 }}>{fmt(scanResult.total)}</span>
                <span style={{ fontFamily:"var(--font-body)", fontSize:12 }}><PaymentBadge payType={scanResult.paymentType} /></span>
              </div>
              <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:12, marginTop:6 }}> {scanResult.address}</div>
              <button onClick={confirmScan} style={{ width:"100%", marginTop:12, background:"linear-gradient(135deg,#00D4FF,#7C3AED)", border:"none", borderRadius:12, padding:14, color:"#fff", fontFamily:"var(--font-head)", fontWeight:700, fontSize:14, cursor:"pointer" }}>
                 Confirm Collection
              </button>
            </div>
          )}
          ...
          {scanned.length > 0 && (
            <>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8, marginTop:8 }}>
                <div style={{ fontFamily:"var(--font-head)", color:"#10B981", fontSize:13, fontWeight:700 }}>Collected ({scanned.length})</div>
                {!removeMode ? (
                  <button onClick={function(){ setRemoveMode(true); }}
                    style={{ background:"rgba(239,68,68,.1)", border:"1px solid rgba(239,68,68,.25)", borderRadius:8, padding:"3px 12px", color:"#EF4444", fontFamily:"var(--font-body)", fontSize:11, cursor:"pointer" }}>
                    Select
                  </button>
                ) : (
                  <div style={{ display:"flex", gap:6 }}>
                    <button onClick={function(){ setBulkSelected(new Set(scanned.map(function(o){ return o.id||o.invoiceNo; }))); }}
                      style={{ background:"rgba(255,255,255,.07)", border:"none", borderRadius:8, padding:"3px 10px", color:"rgba(255,255,255,.5)", fontFamily:"var(--font-body)", fontSize:11, cursor:"pointer" }}>
                      All
                    </button>
                    {bulkSelected.size > 0 && (
                      <button onClick={function(){
                      ...
                      }} style={{ background:"rgba(239,68,68,.2)", border:"1px solid rgba(239,68,68,.4)", borderRadius:8, padding:"3px 10px", color:"#EF4444", fontFamily:"var(--font-body)", fontSize:11, fontWeight:600, cursor:"pointer" }}>
                        Remove ({bulkSelected.size})
                      </button>
                    )}
                    <button onClick={function(){ setRemoveMode(false); setBulkSelected(new Set()); }}
                      style={{ background:"none", border:"none", borderRadius:8, padding:"3px 8px", color:"rgba(255,255,255,.35)", fontFamily:"var(--font-body)", fontSize:11, cursor:"pointer" }}>
                      Cancel
                    </button>
                  </div>
                )}
                ...
                return (
                  <div key={i}
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                      ...
                      <div>
                        <div style={{ fontFamily:"var(--font-body)", color:"#fff", fontSize:13, fontWeight:500 }}>{o.customer}</div>
                        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:11 }}>#{o.invoiceNo} · {o.store}</div>
                        {o.onlineOrderNo ? <div style={{ fontFamily:"var(--font-body)", color:"#00D4FF", fontSize:10, fontWeight:700 }}>Online Order: {o.onlineOrderNo}</div> : null}
                      </div>
                    </div>
                    <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:4 }}>
                      <div style={{ fontFamily:"var(--font-head)", color:"#10B981", fontSize:12, fontWeight:700 }}>{fmt(o.total)}</div>
                      {!removeMode && (
                        <button onClick={function(e){ e.stopPropagation(); if(window.confirm("Remove this order?")) onRemoveOrder && onRemoveOrder(oid); }}
                          style={{ background:"rgba(239,68,68,.12)", border:"1px solid rgba(239,68,68,.25)", borderRadius:6, padding:"2px 8px", color:"#EF4444", fontFamily:"var(--font-body)", fontSize:10, cursor:"pointer" }}>
                          Remove
                        </button>
                      )}
                    </div>
      ...
      {tab === "list" && (
        <>
          {myOrders.length === 0 ? (
            <div style={{ textAlign:"center", color:"rgba(255,255,255,.3)", fontFamily:"var(--font-body)", padding:40, fontSize:14 }}>
            ...
            <>
              ...
              {unscanned.length > 0 && (
                <div style={{ background:"rgba(0,212,255,.07)", border:"1px solid rgba(0,212,255,.2)", borderRadius:14, padding:"12px 14px", marginBottom:12, display:"flex", alignItems:"center", gap:10 }}>
                  ...
                  <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.6)", fontSize:13, flex:1 }}>
                  ...
                  {bulkSelected.size > 0 && (
                    <button onClick={() => {
                      ...
                      style={{ background:"linear-gradient(135deg,#00D4FF,#7C3AED)", border:"none", borderRadius:10, padding:"7px 14px", color:"#fff", fontFamily:"var(--font-head)", fontWeight:700, fontSize:12, cursor:"pointer", whiteSpace:"nowrap" }}>
              ...
              {scanned.length > 0 && unscanned.length === 0 && (
                <div style={{ background:"rgba(16,185,129,.08)", border:"1px solid rgba(16,185,129,.25)", borderRadius:12, padding:"10px 14px", marginBottom:12, textAlign:"center" }}>
                  <span style={{ fontFamily:"var(--font-body)", color:"#10B981", fontSize:13 }}> All orders collected! Head out for delivery.</span>
                </div>
              )}
              {myOrders.map((o) => (
                <DriverOrderRow
        ...
        <div>
          {unscanned.length === 0 ? (
            <div style={{ textAlign:"center", padding:40 }}>
              <div style={{ fontSize:40, marginBottom:10 }}></div>
              <div style={{ fontFamily:"var(--font-head)", color:"#10B981", fontSize:16, fontWeight:700 }}>All Collected!</div>
              <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:13, marginTop:6 }}>You have collected all assigned orders.</div>
            </div>
          ) : (
            <>
              <div style={{ background:"rgba(245,158,11,.08)", border:"1px solid rgba(245,158,11,.2)", borderRadius:14, padding:"12px 14px", marginBottom:12, display:"flex", alignItems:"center", gap:10 }}>
                <span style={{ fontSize:18 }}></span>
                <div style={{ flex:1 }}>
                  <div style={{ fontFamily:"var(--font-head)", color:"#F59E0B", fontSize:13, fontWeight:700 }}>{unscanned.length} orders not yet collected</div>
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12 }}>These orders are waiting at the warehouse</div>
                </div>
              </div>
              {/* Bulk collect all pending */}
              <button onClick={() => { playSound("bulk"); unscanned.forEach(o => onScan(o.id||o.invoiceNo)); }}
                style={{ width:"100%", background:"linear-gradient(135deg,#F59E0B,#FF6B35)", border:"none", borderRadius:12, padding:"13px", color:"#fff", fontFamily:"var(--font-head)", fontWeight:700, fontSize:14, cursor:"pointer", marginBottom:14 }}>
              ...
              {unscanned.map((o) => (
                <div key={o.id||o.invoiceNo} style={{ background:"rgba(245,158,11,.06)", border:"1px solid rgba(245,158,11,.2)", borderRadius:12, padding:"12px 14px", marginBottom:8, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontFamily:"var(--font-body)", color:"#fff", fontSize:13, fontWeight:600 }}>{o.customer}</div>
                    <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:11 }}>#{o.invoiceNo}{o.onlineOrderNo?" · Online Order: "+o.onlineOrderNo:""} · {o.store}</div>
                    <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:11 }}> {o.address?.slice(0,40)}{o.address?.length>40?"...":""}</div>
                  </div>
                  <div style={{ textAlign:"right", flexShrink:0, marginLeft:8 }}>
                    <div style={{ fontFamily:"var(--font-head)", color:"#FF6B35", fontSize:13, fontWeight:700 }}>{fmt(o.total)}</div>
                    <div style={{ fontFamily:"var(--font-body)", fontSize:10, marginTop:2 }}><PaymentBadge payType={o.paymentType} small /></div>
                    <button onClick={() => { playSound('collect'); onScan(o.id||o.invoiceNo); }}
                      style={{ marginTop:6, background:"rgba(0,212,255,.15)", border:"1px solid rgba(0,212,255,.3)", borderRadius:8, padding:"4px 10px", color:"#00D4FF", fontFamily:"var(--font-head)", fontWeight:700, fontSize:11, cursor:"pointer" }}>
                      Collect
                    </button>
                  </div>
                </div>
      ...
      <div style={{ padding:"11px 14px", display:"flex", alignItems:"center", gap:10 }}>
        ...
        <div onClick={function() { setExp(function(e) { return !e; }); }} style={{ flex:1, minWidth:0, cursor:"pointer" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ fontFamily:"var(--font-body)", color:"#fff", fontSize:13, fontWeight:600 }}>{order.customer}</div>
              <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:11 }}>
                {"#" + order.invoiceNo + "   " + order.store}
              </div>
              {order.onlineOrderNo ? (
                <div style={{ fontFamily:"var(--font-body)", color:"#FF6B35", fontSize:11, fontWeight:600 }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:6, flexShrink:0 }}>
              <span style={{ fontFamily:"var(--font-head)", color:"#FF6B35", fontSize:13, fontWeight:700 }}>{fmt(order.total)}</span>
              {(order.extraAmount > 0) && <span style={{ fontFamily:"var(--font-head)", color:"#EA580C", fontSize:11, fontWeight:700 }}>{"+"+fmt(order.extraAmount)}</span>}
        ...
        <div style={{ borderTop:"1px solid rgba(255,255,255,.06)", padding:"12px 14px", background:"rgba(0,0,0,.18)" }}>
          {order.onlineOrderNo ? (
            <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:6 }}>
              <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:11 }}>Online Order No:</span>
              <span style={{ fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:12, fontWeight:700 }}>{order.onlineOrderNo}</span>
            </div>
          ) : null}
          <div style={{ display:"flex", alignItems:"center", flexWrap:"wrap", gap:6, marginBottom:6  }}>{(function(){ var _a = detectArea(order.address); return _a ? <span style={{ background:"rgba(0,212,255,.12)", border:"1px solid rgba(0,212,255,.25)", borderRadius:20, padding:"1px 9px", fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:10, fontWeight:700, marginRight:6 }}>{_a}</span> : null; })()}<span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:12  }}>{order.address}</span></div>
          <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:12, marginBottom:8 }}>{"📞 " + order.phone}</div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:10 }}>
            <button onClick={function(e){e.preventDefault();window.location.href=callHref;}} style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:7, background:"rgba(16,185,129,.12)", border:"1px solid rgba(16,185,129,.3)", borderRadius:10, padding:"10px 8px", textDecoration:"none" }}>
              ...
              <span style={{ fontFamily:"var(--font-head)", color:"#10B981", fontSize:12, fontWeight:700 }}>Call</span>
            </button>
            <button onClick={function(e){ e.preventDefault(); var callUrl = "whatsapp://call?phone=" + waPhone; window.location.href = callUrl; }} style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:7, background:"rgba(37,211,102,.12)", border:"1px solid rgba(37,211,102,.35)", borderRadius:10, padding:"10px 8px" }}>
              <span style={{ fontSize:16 }}></span>
              <span style={{ fontFamily:"var(--font-head)", color:"#25D366", fontSize:12, fontWeight:700 }}>WA Call</span>
            </button>
          </div>
          <div style={{ background:"rgba(37,211,102,.07)", border:"1px solid rgba(37,211,102,.2)", borderRadius:10, padding:"10px 12px", marginBottom:10 }}>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:10, marginBottom:6, textTransform:"uppercase", letterSpacing:.5 }}>Out-for-Delivery Notification</div>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.75)", fontSize:11, lineHeight:1.6, marginBottom:8 }}>
            ...
            <button onClick={function(){window.open(waHref,"_blank");}} style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:7, width:"100%", background:"linear-gradient(135deg,#25D366,#128C7E)", borderRadius:10, padding:"11px", textDecoration:"none" }}>
              <span style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:13, fontWeight:700 }}>Notify - Out for Delivery</span>
            </button>
          </div>
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            <span style={{ fontFamily:"var(--font-body)", fontSize:11 }}><PaymentBadge payType={order.paymentType} small /></span>
            {order.onlineOrderNo && <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11 }}>{"Online Order: " + order.onlineOrderNo}</span>}
          </div>
          {!order.scanned && onTransfer && (
            <button onClick={function(e) { e.stopPropagation(); onTransfer(order); }}
              style={{ width:"100%", marginTop:10, background:"rgba(255,107,53,.1)", border:"1px solid rgba(255,107,53,.3)", borderRadius:10, padding:"9px", color:"#FF6B35", fontFamily:"var(--font-head)", fontWeight:700, fontSize:12, cursor:"pointer" }}>
...
function DriverManualOrderForm({ driverId, onAdd }) {
  ...
  if (!show) return (
    <div style={{ display:"flex", gap:8, marginBottom:10 }}>
      <button onClick={function(){ setShow(true); }}
        style={{ flex:1, background:"rgba(255,107,53,.07)", border:"1px dashed rgba(255,107,53,.35)", borderRadius:12, padding:"11px", color:"#FF6B35", fontFamily:"var(--font-head)", fontSize:13, fontWeight:700, cursor:"pointer" }}>
        + Add Manual Order
      </button>
      <LabelScanner
        onExtracted={function(fields){ fillFromScan(fields); }}
  ...
  return (
    <div style={{ background:"rgba(255,255,255,.04)", border:"1px solid rgba(255,107,53,.3)", borderRadius:14, padding:14, marginBottom:10 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
        <div style={{ fontFamily:"var(--font-head)", color:"#FF6B35", fontSize:14, fontWeight:800 }}>Add Manual Order</div>
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          <LabelScanner
            onExtracted={fillFromScan}
            onError={function(msg){ setScanMsg("⚠️ " + msg); }}
      ...
      {scanMsg && (
        <div style={{ background: scanMsg.startsWith("✅") ? "rgba(16,185,129,.1)" : "rgba(239,68,68,.1)", border:"1px solid " + (scanMsg.startsWith("✅") ? "rgba(16,185,129,.3)" : "rgba(239,68,68,.3)"), borderRadius:10, padding:"8px 12px", fontFamily:"var(--font-body)", fontSize:12, color: scanMsg.startsWith("✅") ? "#10B981" : "#EF4444", marginBottom:10 }}>
          {scanMsg}
        </div>
      )}
      ...
      <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:8 }}>
        {["Trikart Online","Webstore Online","ReStore Online","Other"].map(function(s){
          return <button key={s} onClick={function(){ setStore(s); if(s!=="Other") setCustomStoreName(""); }}
            style={{ background:store===s?"rgba(0,212,255,.15)":"rgba(255,255,255,.06)", border:store===s?"1.5px solid #00D4FF":"1px solid rgba(255,255,255,.1)", borderRadius:20, padding:"5px 10px", color:store===s?"#00D4FF":"rgba(255,255,255,.5)", fontFamily:"var(--font-body)", fontSize:11, cursor:"pointer" }}>{s}</button>;
        })}
      </div>
      {store === "Other" && (
        <div style={{ marginBottom:12 }}>
      ...
      {[
        ...
        return (
          <div key={f[0]} style={{ marginBottom:8 }}>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:10, marginBottom:3 }}>{f[0].toUpperCase()}</div>
            <input type={f[3]} value={f[1]} onChange={function(e){ f[2](e.target.value); }} placeholder={f[4]}
              style={{ width:"100%", boxSizing:"border-box", background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,255,255,.1)", borderRadius:10, padding:"9px 12px", color:"#fff", fontFamily:"var(--font-body)", fontSize:13, outline:"none" }} />
          </div>
        );
      })}
      ...
      <div style={{ marginBottom:8 }}>
        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:10, marginBottom:3 }}>AREA (KUWAIT)</div>
        <input type="text" value={areaSearch}
          onChange={function(e){ setAreaSearch(e.target.value); setArea(""); }}
          placeholder="Type to search area..."
          style={{ width:"100%", boxSizing:"border-box", background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,255,255,.1)", borderRadius:area||!areaSearch?"10px":"10px 10px 0 0", padding:"9px 12px", color:"#fff", fontFamily:"var(--font-body)", fontSize:13, outline:"none" }} />
        {area ? (
          <div style={{ background:"rgba(0,212,255,.1)", border:"1px solid rgba(0,212,255,.3)", borderRadius:10, padding:"7px 12px", display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:4 }}>
            <span style={{ fontFamily:"var(--font-body)", color:"#00D4FF", fontSize:12, fontWeight:600 }}>{area}</span>
            <button onClick={function(){ setArea(""); setAreaSearch(""); }} style={{ background:"none", border:"none", color:"rgba(255,255,255,.4)", cursor:"pointer", fontSize:14 }}>✕</button>
          </div>
        ) : areaSearch ? (
          <div style={{ background:"#1A2035", border:"1px solid rgba(255,255,255,.15)", borderTop:"none", borderRadius:"0 0 10px 10px", maxHeight:150, overflowY:"auto" }}>
            {ALL_AREAS.filter(function(a){ return a.toLowerCase().includes(areaSearch.toLowerCase()); }).slice(0,15).map(function(a){
              return <div key={a} onClick={function(){ setArea(a); setAreaSearch(a); }}
                style={{ padding:"7px 12px", fontFamily:"var(--font-body)", color:"rgba(255,255,255,.8)", fontSize:12, cursor:"pointer", borderBottom:"1px solid rgba(255,255,255,.05)" }}>{a}</div>;
            })}
          </div>
        ) : null}
      </div>
      ...
      {/* Payment */}
      <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:10, marginBottom:6, marginTop:4 }}>PAYMENT</div>
      <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:12 }}>
        {PAYS.map(function(p){
          return <button key={p} onClick={function(){ setPay(p); }}
            style={{ background:pay===p?"rgba(16,185,129,.15)":"rgba(255,255,255,.06)", border:pay===p?"1.5px solid #10B981":"1px solid rgba(255,255,255,.1)", borderRadius:20, padding:"4px 10px", color:pay===p?"#10B981":"rgba(255,255,255,.5)", fontFamily:"var(--font-body)", fontSize:11, cursor:"pointer" }}>{p}</button>;
        })}
      </div>

      {err && <div style={{ color:"#EF4444", fontFamily:"var(--font-body)", fontSize:12, marginBottom:8 }}>{err}</div>}
      <button onClick={submit}
        style={{ width:"100%", background:"linear-gradient(135deg,#FF6B35,#FF3D71)", border:"none", borderRadius:12, padding:"12px", color:"#fff", fontFamily:"var(--font-head)", fontSize:13, fontWeight:700, cursor:"pointer" }}>
        Add to My Orders
      </button>
    </div>
  );
    ...
    <div style={{ flex:1, display:"flex", flexDirection:"column", minHeight:0, position:"relative" }}>
      ...
      <div style={{ position:"relative", marginBottom:10 }}>
        ...
        <input
          ...
          style={{ width:"100%", boxSizing:"border-box", background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,255,255,.15)", borderRadius:12, padding:"11px 40px 11px 40px", color:"#fff", fontFamily:"var(--font-body)", fontSize:13, outline:"none" }}
        />
        {searchQ ? (
        ...
        {[["",counts.collected,"Collected","#00D4FF"],["",counts.delivered,"Delivered","#10B981"],["",counts.postponed,"Postponed","#8B5CF6"],["x",counts.cancelled,"Cancelled","#EF4444"]].map(function(item) {
          ...
          return (
            <div key={l} style={{ background:c+"10", border:"1px solid "+c+"25", borderRadius:12, padding:"10px 6px", textAlign:"center" }}>
              <div style={{ fontFamily:"var(--font-head)", color:c, fontSize:18, fontWeight:800 }}>{v}</div>
              <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:10 }}>{l}</div>
            </div>
          );
        })}
      </div>
      ...
      {(storeFilter !== "all" || payFilter !== "all") && (
        <div style={{ background:"rgba(255,255,255,.05)", borderRadius:10, padding:"8px 12px", marginBottom:12, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:12 }}>Showing {filtered.length} of {myOrders.length} orders</span>
          <button onClick={function() { setStoreFilter("all"); setPayFilter("all"); }} style={{ background:"none", border:"none", color:"#00D4FF", fontFamily:"var(--font-body)", fontSize:12, cursor:"pointer" }}>Clear x</button>
        </div>
      )}
          ...
          {!rearranging ? (
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <button onClick={function(){ setRearranging(true); setTouchOrder([]); }}
                style={{ background:"rgba(124,58,237,.12)", border:"1px solid rgba(124,58,237,.3)", borderRadius:10, padding:"6px 14px", color:"#A78BFA", fontFamily:"var(--font-head)", fontSize:12, fontWeight:700, cursor:"pointer", display:"flex", alignItems:"center", gap:6 }}>
              ...
              {Object.keys(orderSeq).length > 0 && (
                <button onClick={function(){ setOrderSeq({}); try{ localStorage.removeItem("df_route_seq_" + driverId); }catch(e){} }}
                  style={{ background:"none", border:"none", color:"rgba(255,255,255,.25)", fontFamily:"var(--font-body)", fontSize:11, cursor:"pointer" }}>Reset order</button>
              )}
            </div>
          ) : (
            <div>
              <div style={{ display:"flex", gap:8, marginBottom:8 }}>
                <div style={{ flex:1, background:"rgba(124,58,237,.1)", border:"1px solid rgba(124,58,237,.25)", borderRadius:10, padding:"8px 12px" }}>
                  <div style={{ fontFamily:"var(--font-head)", color:"#A78BFA", fontSize:12, fontWeight:700 }}>Tap orders in delivery order</div>
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:11, marginTop:2 }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                  ...
                  }} style={{ background:"linear-gradient(135deg,#00D4FF,#7C3AED)", border:"none", borderRadius:10, padding:"7px 14px", color:"#fff", fontFamily:"var(--font-head)", fontSize:12, fontWeight:700, cursor:"pointer" }}>
                    Done
                  </button>
                  <button onClick={function(){ setRearranging(false); setTouchOrder([]); }}
                    style={{ background:"rgba(255,255,255,.07)", border:"none", borderRadius:10, padding:"7px 10px", color:"rgba(255,255,255,.4)", fontFamily:"var(--font-body)", fontSize:11, cursor:"pointer" }}>
                    Cancel
                  </button>
                </div>
              </div>
              {touchOrder.length > 0 && (
                <button onClick={function(){ setTouchOrder([]); }}
                  style={{ background:"none", border:"none", color:"rgba(255,255,255,.2)", fontFamily:"var(--font-body)", fontSize:11, cursor:"pointer" }}>
                  ↺ Start over
                </button>
              )}
            </div>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign:"center", color:"rgba(255,255,255,.3)", fontFamily:"var(--font-body)", padding:40 }}>
            ...
            return (
              <div key={oid}
                ...
                <div style={{ width:36, height:36, borderRadius:10, background:isTapped?"rgba(0,212,255,.2)":"rgba(255,255,255,.06)", border:"1.5px solid "+(isTapped?"#00D4FF":"rgba(255,255,255,.15)"), display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                  {isTapped ? (
                    <span style={{ fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:16, fontWeight:900 }}>{tapIdx+1}</span>
                  ) : (
                    <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.2)", fontSize:11 }}>tap</span>
                  )}
                </div>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontFamily:"var(--font-head)", color:isTapped?"#fff":"rgba(255,255,255,.6)", fontSize:13, fontWeight:700 }}>{o.customer}</div>
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.3)", fontSize:11 }}>#{o.invoiceNo} · {o.store}{o.onlineOrderNo?" · Online Order: "+o.onlineOrderNo:""}</div>
                  <div style={{ display:"flex", gap:6, marginTop:3, flexWrap:"wrap" }}>
                    <span style={{ fontFamily:"var(--font-head)", color:"#FF6B35", fontSize:12, fontWeight:700 }}>{fmt(o.total)}</span>
                    {area && <span style={{ background:"rgba(0,212,255,.12)", border:"1px solid rgba(0,212,255,.2)", borderRadius:20, padding:"1px 8px", fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:10, fontWeight:700 }}>{area}</span>}
                    {orderTags && orderTags[o.invoiceNo] && <span style={{ background:"rgba(245,158,11,.15)", borderRadius:20, padding:"1px 8px", fontFamily:"var(--font-head)", color:"#F59E0B", fontSize:10, fontWeight:700 }}>{orderTags[o.invoiceNo]}</span>}
                  </div>
                </div>
...
function TagButton({ invoiceNo, orderTags, onSetTag }) {
  ...
  if (!editing) {
    return (
      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
        {currentTag ? (
          <span style={{ background:"rgba(245,158,11,.15)", border:"1px solid rgba(245,158,11,.35)", borderRadius:20, padding:"3px 12px", fontFamily:"var(--font-head)", color:"#F59E0B", fontSize:12, fontWeight:700 }}>
            {currentTag}
          </span>
        ) : null}
        <button onClick={function(){ setVal(currentTag||""); setEditing(true); }}
          style={{ background:"rgba(255,255,255,.06)", border:"1px solid rgba(255,255,255,.12)", borderRadius:20, padding:"3px 12px", fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:11, cursor:"pointer" }}>
          {currentTag ? "Edit Tag" : "+ Add Tag"}
        </button>
        {currentTag && (
          <button onClick={function(){ if(onSetTag) onSetTag(invoiceNo, null); }}
            style={{ background:"none", border:"none", color:"rgba(255,255,255,.3)", fontFamily:"var(--font-body)", fontSize:11, cursor:"pointer" }}>Remove</button>
        )}
      </div>
    );
  }
  return (
    <div style={{ background:"rgba(245,158,11,.06)", border:"1px solid rgba(245,158,11,.2)", borderRadius:12, padding:"12px 14px", marginBottom:8 }}>
      <div style={{ fontFamily:"var(--font-head)", color:"#F59E0B", fontSize:12, fontWeight:700, marginBottom:8 }}>Tag this order</div>
      <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginBottom:10 }}>
        {PRESET_TAGS.map(function(t) {
          return (
            <button key={t} onClick={function(){ if(onSetTag) onSetTag(invoiceNo, t); setEditing(false); }}
              style={{ background: currentTag===t?"rgba(245,158,11,.25)":"rgba(255,255,255,.06)", border:"1px solid "+(currentTag===t?"rgba(245,158,11,.5)":"rgba(255,255,255,.1)"), borderRadius:20, padding:"4px 12px", fontFamily:"var(--font-body)", color:currentTag===t?"#F59E0B":"rgba(255,255,255,.6)", fontSize:12, cursor:"pointer" }}>
              {t}
            </button>
          );
        })}
      ...
      <div style={{ display:"flex", gap:8 }}>
        <input value={val} onChange={function(e){ setVal(e.target.value); }} placeholder="Custom tag name..."
          style={{ flex:1, background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,255,255,.15)", borderRadius:10, padding:"8px 12px", color:"#fff", fontFamily:"var(--font-body)", fontSize:13, outline:"none" }} />
        <button onClick={function(){ if(val.trim() && onSetTag) onSetTag(invoiceNo, val.trim()); setEditing(false); }}
          style={{ background:"rgba(245,158,11,.2)", border:"1px solid rgba(245,158,11,.4)", borderRadius:10, padding:"8px 14px", color:"#F59E0B", fontFamily:"var(--font-head)", fontSize:12, fontWeight:700, cursor:"pointer" }}>
          Set
        </button>
        <button onClick={function(){ setEditing(false); }}
          style={{ background:"rgba(255,255,255,.05)", border:"none", borderRadius:10, padding:"8px 10px", color:"rgba(255,255,255,.4)", fontFamily:"var(--font-body)", fontSize:12, cursor:"pointer" }}>
          Cancel
        </button>
      </div>
    </div>
...
function EditOrderModal({ order, onSave, onClose }) {
  ...
  const inputStyle = { width:"100%", boxSizing:"border-box", background:"rgba(255,255,255,.07)", border:"1px solid rgba(0,212,255,.25)", borderRadius:12, padding:"12px 14px", color:"#fff", fontFamily:"var(--font-body)", fontSize:14, outline:"none" };
  const labelStyle = { fontFamily:"var(--font-body)", color:"rgba(255,255,255,.45)", fontSize:11, marginBottom:5, textTransform:"uppercase", letterSpacing:.5, display:"block" };
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.88)", zIndex:9100, display:"flex", alignItems:"flex-end" }}>
      <div style={{ width:"100%", background:"#0F1629", borderRadius:"24px 24px 0 0", padding:"24px 20px 32px", maxHeight:"85dvh", overflowY:"auto" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
          <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:17, fontWeight:700 }}>✏️ Edit Order Details</div>
          <button onClick={onClose} style={{ background:"rgba(255,255,255,.08)", border:"none", borderRadius:20, padding:"5px 12px", color:"rgba(255,255,255,.6)", fontFamily:"var(--font-body)", fontSize:12, cursor:"pointer" }}>✕</button>
        </div>
        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12, marginBottom:20 }}>
          #{order.invoiceNo} · {order.store}
        </div>
        ...
        <div style={{ display:"flex", gap:10, marginTop:8 }}>
          <button onClick={onClose} style={{ flex:1, background:"rgba(255,255,255,.07)", border:"none", borderRadius:12, padding:13, color:"#fff", fontFamily:"var(--font-body)", cursor:"pointer" }}>
            Cancel
          </button>
          <button onClick={save} style={{ flex:2, background:"linear-gradient(135deg,#00D4FF,#7C3AED)", border:"none", borderRadius:12, padding:13, color:"#fff", fontFamily:"var(--font-head)", fontWeight:700, fontSize:14, cursor:"pointer" }}>
            Save Changes
          </button>
        </div>
      </div>
    ...
    <div style={{ background:bgColor, border:"1px solid " + borderColor, borderRadius:14, marginBottom:10, overflow:"hidden" }}>
      <div onClick={function() { setExp(function(v) { return !v; }); }} style={{ padding:"13px 15px", cursor:"pointer" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:5 }}>
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:14, fontWeight:700 }}>{order.customer}</div>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:11, marginTop:1 }}>
              {"#" + order.invoiceNo + "   " + order.store}
            </div>
            {(function(){
                ...
                return (
                  <div style={{ display:"flex", alignItems:"center", gap:4, marginTop:3 }} onClick={function(e){ e.stopPropagation(); }}>
                    <input autoFocus type="text" value={ooVal} onChange={function(e){ setOoVal(e.target.value); }}
                      ...
                      style={{ width:90, background:"rgba(0,212,255,.1)", border:"1px solid #00D4FF", borderRadius:6, padding:"2px 6px", color:"#00D4FF", fontFamily:"var(--font-body)", fontSize:11, outline:"none" }} />
                    <button onClick={function(e){ e.stopPropagation(); setEditingOO(false); if(ooVal.trim()&&onSaveOO) onSaveOO(order.invoiceNo, ooVal.trim()); }}
                      style={{ background:"#00D4FF", border:"none", borderRadius:4, padding:"2px 6px", color:"#000", fontFamily:"var(--font-body)", fontSize:10, fontWeight:700, cursor:"pointer" }}>✓</button>
                  </div>
                );
              }
              if (order.onlineOrderNo) {
                return (
                  <div style={{ display:"flex", alignItems:"center", gap:4, marginTop:3 }}>
                    <span style={{ background:"rgba(0,212,255,.15)", borderRadius:6, padding:"1px 7px", border:"1px solid rgba(0,212,255,.3)", fontFamily:"var(--font-body)", color:"#00D4FF", fontSize:11, fontWeight:700 }}>
                      Online Order: {order.onlineOrderNo}
                    </span>
                  </div>
                );
              ...
              return (
                <button onClick={function(e){ e.stopPropagation(); setEditingOO(true); }}
                  style={{ marginTop:3, background:"rgba(255,255,255,.06)", border:"1px dashed rgba(255,255,255,.2)", borderRadius:6, padding:"1px 8px", color:"rgba(255,255,255,.35)", fontFamily:"var(--font-body)", fontSize:10, cursor:"pointer" }}>
                  + Online Order
                </button>
              );
            })()}
        ...
        <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
          <span style={{ fontFamily:"var(--font-head)", color:"#FF6B35", fontSize:13, fontWeight:700 }}>{fmt(order.total)}</span>
          {(order.extraAmount > 0) && (
            <span style={{ fontFamily:"var(--font-head)", color:"#EA580C", fontSize:11, fontWeight:700, background:"rgba(234,88,12,.12)", borderRadius:20, padding:"2px 8px", border:"1px solid rgba(234,88,12,.3)" }}>+{fmt(order.extraAmount)} extra</span>
          )}
          <span style={{ fontFamily:"var(--font-body)", fontSize:11 }}><PaymentBadge payType={order.paymentType} small /></span>
          {isExchangeOrder && (
            <span style={{ background:"rgba(107,114,128,.2)", color:"#9CA3AF", borderRadius:20, padding:"2px 10px", fontSize:11, fontFamily:"var(--font-head)", fontWeight:700, border:"1px solid rgba(107,114,128,.3)" }}>Exchange</span>
          )}
          {(function(){ var _a = detectArea(order.address); return _a ? (
            <span style={{ background:"rgba(0,212,255,.12)", border:"1px solid rgba(0,212,255,.3)", borderRadius:20, padding:"2px 10px", fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:11, fontWeight:700 }}>{_a}</span>
          ) : null; })()}
          {orderTags && orderTags[order.invoiceNo] && (
            <span style={{ background:"rgba(245,158,11,.15)", border:"1px solid rgba(245,158,11,.35)", borderRadius:20, padding:"2px 10px", fontFamily:"var(--font-head)", color:"#F59E0B", fontSize:11, fontWeight:700 }}>{orderTags[order.invoiceNo]}</span>
          )}
        </div>
      </div>
      ...
      {exp && (
        <div style={{ borderTop:"1px solid rgba(255,255,255,.06)", padding:"14px 15px", background:"rgba(0,0,0,.15)" }}>
          <div style={{ display:"flex", alignItems:"center", flexWrap:"wrap", gap:6, marginBottom:6  }}>{(function(){ var _a = detectArea(order.address); return _a ? <span style={{ background:"rgba(0,212,255,.12)", border:"1px solid rgba(0,212,255,.25)", borderRadius:20, padding:"1px 9px", fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:10, fontWeight:700, marginRight:6 }}>{_a}</span> : null; })()}<span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:12  }}>{order.address}</span></div>
          <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:12, marginBottom:10 }}>{"📞 " + order.phone}</div>
          ...
          {isExchangeOrder && (
            <div style={{ background:"rgba(107,114,128,.1)", border:"1px solid rgba(107,114,128,.3)", borderRadius:12, padding:"12px 14px", marginBottom:12 }}>
              <div style={{ fontFamily:"var(--font-head)", color:"#9CA3AF", fontSize:13, fontWeight:700 }}>Exchange Order - No cash collection required.</div>
            </div>
          )}
          ...
          {helpOpen && (
            <div style={{ background:"rgba(239,68,68,.06)", border:"1px solid rgba(239,68,68,.2)", borderRadius:12, padding:14, marginBottom:10 }}>
              <div style={{ fontFamily:"var(--font-head)", color:"#EF4444", fontSize:13, fontWeight:700, marginBottom:10 }}>Request Help - Select Issue</div>
              {[
                ...
                return (
                  <button key={h.key} onClick={function() {
                    ...
                    <span style={{ background:"rgba(239,68,68,.15)", color:"#EF4444", borderRadius:6, padding:"2px 6px", fontFamily:"var(--font-head)", fontSize:10, fontWeight:700, flexShrink:0 }}>{h.icon}</span>
                    <span style={{ fontFamily:"var(--font-body)", color:"#fff", fontSize:13 }}>{h.label}</span>
                  </button>
                );
              })}
              <button onClick={function(){ setHelpOpen(false); }} style={{ width:"100%", background:"none", border:"none", color:"rgba(255,255,255,.35)", fontFamily:"var(--font-body)", fontSize:12, padding:"6px 0", cursor:"pointer" }}>Cancel</button>
            </div>
          )}

          <div style={{ display:"flex", gap:8, marginBottom:8 }}>
            {!helpOpen && (
              <button onClick={function(){ setHelpOpen(true); }} style={{ flex:1, background:"rgba(239,68,68,.1)", border:"1px solid rgba(239,68,68,.25)", borderRadius:12, padding:12, color:"#EF4444", fontFamily:"var(--font-head)", fontWeight:700, fontSize:13, cursor:"pointer" }}>
                Need Help
              </button>
            )}
            {(isActive || isPostponed) && (
              <button onClick={onUpdate} style={{ flex:2, background:"linear-gradient(135deg,#00D4FF,#7C3AED)", border:"none", borderRadius:12, padding:13, color:"#fff", fontFamily:"var(--font-head)", fontWeight:700, fontSize:14, cursor:"pointer" }}>
          ...
          {order.note && (
            <div style={{ background:"rgba(255,255,255,.04)", borderRadius:10, padding:"8px 12px", marginBottom:8 }}>
              <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11 }}>Note: {order.note}</div>
            </div>
          )}
          {/* Tag */}
          ...
          {onOpenTransfer && !isDelivered && (
            <button onClick={function() { onOpenTransfer(order); }}
              style={{ width:"100%", background:"rgba(255,107,53,.1)", border:"1px solid rgba(255,107,53,.3)", borderRadius:12, padding:11, color:"#FF6B35", fontFamily:"var(--font-head)", fontWeight:700, fontSize:13, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:7 }}>
              Request Transfer to Another Driver
            </button>
          )}
          ...
          <button onClick={function(e) { e.stopPropagation(); setEditOpen(true); }}
            style={{ width:"100%", marginTop:8, background:"rgba(0,212,255,.08)", border:"1px solid rgba(0,212,255,.25)", borderRadius:12, padding:11, color:"#00D4FF", fontFamily:"var(--font-head)", fontWeight:700, fontSize:13, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:7 }}>
    ...
    <div style={{ position:"fixed", top:0, left:0, right:0, bottom:0, background:"rgba(0,0,0,.88)", zIndex:9000, display:"flex", flexDirection:"column", justifyContent:"flex-end" }}>
      <div style={{ width:"100%", background:"#0F1629", borderRadius:"24px 24px 0 0", maxHeight:"92dvh", display:"flex", flexDirection:"column", WebkitOverflowScrolling:"touch" }}>
        ...
        <div style={{ flex:1, overflowY:"auto", WebkitOverflowScrolling:"touch", padding:"24px 20px 16px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
            <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:17, fontWeight:700 }}>Update Delivery Status</div>
            <button onClick={onClose} style={{ background:"rgba(255,255,255,.08)", border:"none", borderRadius:20, padding:"5px 12px", color:"rgba(255,255,255,.6)", fontFamily:"var(--font-body)", fontSize:12, cursor:"pointer" }}>✕ Close</button>
          </div>
          <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:13, marginBottom:18 }}>
            ...
            {[["delivered","Delivered","#10B981"],["postponed","Postponed","#8B5CF6"],["cancelled","Cancelled","#EF4444"]].map(function(item) {
              ...
              return (
                <button key={s} onClick={function(){ setStatus(s); setPayMode(""); setLinkPlatform(""); }}
                  ...
                  <div style={{ fontFamily:"var(--font-head)", color:status===s?c:"#fff", fontSize:14, fontWeight:700 }}>{l}</div>
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11, marginTop:2 }}>
          ...
          {status === "delivered" && isCOD && (
            <div style={{ background:"rgba(245,158,11,.06)", border:"1px solid rgba(245,158,11,.2)", borderRadius:14, padding:"14px", marginBottom:16 }}>
              <div style={{ fontFamily:"var(--font-head)", color:"#F59E0B", fontSize:13, fontWeight:700, marginBottom:10 }}>
                ...
                {[
                  ...
                  return (
                    <button key={opt.id} onClick={function(){ setPayMode(opt.id); setLinkPlatform(""); setSplitLink(""); setCashAmt(""); }}
                      ...
                      <div style={{ fontFamily:"var(--font-head)", color:payMode===opt.id?opt.color:"rgba(255,255,255,.6)", fontSize:10, fontWeight:700 }}>{opt.sub}</div>
                    </button>
                  );
                })}
              </div>
              ...
              {payMode === "full_link" && (
                <div>
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11, marginBottom:8 }}>SELECT PLATFORM</div>
                  <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                    {LINK_OPTIONS.map(function(lk) {
                      return (
                        <button key={lk.id} onClick={function(){ setLinkPlatform(lk.id); }}
                          ...
                          <div style={{ fontFamily:"var(--font-head)", color:linkPlatform===lk.id?lk.color:"rgba(255,255,255,.6)", fontSize:11, fontWeight:700 }}>{lk.label}</div>
                        </button>
                      );
                    })}
                  </div>
                  {linkPlatform && <div style={{ fontFamily:"var(--font-body)", color:"#A855F7", fontSize:12, marginTop:8 }}>✓ Full {fmt(total)} via {linkPlatform}</div>}
                </div>
              )}
              ...
              {payMode === "split" && (
                <div>
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11, marginBottom:8 }}>CASH AMOUNT RECEIVED</div>
                  <input type="number" value={cashAmt} onChange={function(e){ setCashAmt(e.target.value); }} placeholder={"e.g. " + fmt(total/2)}
                    style={{ width:"100%", background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,255,255,.15)", borderRadius:10, padding:"10px 14px", color:"#fff", fontFamily:"var(--font-body)", fontSize:14, marginBottom:10, boxSizing:"border-box" }} />
                  {cashNum > 0 && cashNum < total && (
                    <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12, marginBottom:10 }}>
                      Link amount: <span style={{ color:"#00D4FF", fontWeight:600 }}>{fmt(linkNum)}</span>
                    </div>
                  )}
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11, marginBottom:8 }}>LINK PLATFORM</div>
                  <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                    {LINK_OPTIONS.map(function(lk) {
                      return (
                        <button key={lk.id} onClick={function(){ setSplitLink(lk.id); }}
                          ...
                          <div style={{ fontFamily:"var(--font-head)", color:splitLink===lk.id?lk.color:"rgba(255,255,255,.6)", fontSize:11, fontWeight:700 }}>{lk.label}</div>
                        </button>
                      );
                    })}
                  </div>
                  {splitValid && <div style={{ fontFamily:"var(--font-body)", color:"#00D4FF", fontSize:12, marginTop:8 }}>✓ Cash {fmt(cashNum)} + {splitLink} {fmt(linkNum)}</div>}
                </div>
              )}
            </div>
          )}
          ...
          {status === "delivered" && isCOD && (
            <div style={{ marginBottom:14 }}>
              <button onClick={function(){ setPartialMode(function(v){ return !v; }); setActualAmt(""); setPartialReason(""); }}
                ...
                <div style={{ fontFamily:"var(--font-head)", color:partialMode?"#EF4444":"rgba(255,255,255,.6)", fontSize:13, fontWeight:700 }}>
              ...
              {partialMode && (
                <div style={{ background:"rgba(239,68,68,.06)", border:"1px solid rgba(239,68,68,.2)", borderRadius:12, padding:14, marginTop:8 }}>
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:11, marginBottom:6 }}>ACTUAL AMOUNT COLLECTED (KD)</div>
                  <input type="number" value={actualAmt} onChange={function(e){ setActualAmt(e.target.value); }}
                    placeholder={"Less than " + total.toFixed(3)}
                    style={{ width:"100%", boxSizing:"border-box", background:"rgba(255,255,255,.08)", border:"1px solid rgba(239,68,68,.3)", borderRadius:10, padding:"10px 12px", color:"#EF4444", fontFamily:"var(--font-head)", fontSize:16, fontWeight:700, outline:"none", marginBottom:8 }} />
                  {actualNum > 0 && actualNum < total && (
                    <div style={{ fontFamily:"var(--font-body)", fontSize:12, marginBottom:8 }}>
                  ...
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:11, marginBottom:6, marginTop:4 }}>REASON</div>
                  <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:partialReason==="Other"?8:0 }}>
                    {["Refused delivery charge","Took 1 item only","Price dispute","Other"].map(function(r){
                      return <button key={r} onClick={function(){ setPartialReason(r); }}
                        style={{ background:partialReason===r?"rgba(239,68,68,.2)":"rgba(255,255,255,.06)", border:partialReason===r?"1px solid #EF4444":"1px solid rgba(255,255,255,.1)", borderRadius:20, padding:"4px 10px", color:partialReason===r?"#EF4444":"rgba(255,255,255,.5)", fontFamily:"var(--font-body)", fontSize:11, cursor:"pointer" }}>{r}</button>;
                    })}
                  </div>
                  {partialReason === "Other" && (
                    <input type="text" value={otherReason}
                      ...
                      style={{ width:"100%", boxSizing:"border-box", background:"rgba(255,255,255,.07)", border:"1px solid rgba(239,68,68,.3)", borderRadius:8, padding:"8px 12px", color:"#fff", fontFamily:"var(--font-body)", fontSize:12, outline:"none", marginTop:6 }} />
                  )}
                </div>
              )}
            </div>
          ...
          {status === "delivered" && (
            <div style={{ background:"rgba(255,165,0,.06)", border:"1px solid rgba(255,165,0,.2)", borderRadius:14, padding:14, marginBottom:14 }}>
              <div style={{ fontFamily:"var(--font-head)", color:"#FFA500", fontSize:13, fontWeight:700, marginBottom:4 }}>➕ Extra Amount to Collect?</div>
              <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12, marginBottom:10 }}>
              ...
              <input type="number" value={extraAmt} onChange={function(e){ setExtraAmt(e.target.value); }}
                ...
                style={{ width:"100%", boxSizing:"border-box", background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,165,0,.3)", borderRadius:10, padding:"10px 14px", color:"#FFA500", fontFamily:"var(--font-head)", fontSize:15, fontWeight:700, outline:"none", marginBottom: (parseFloat(extraAmt)||0)>0 ? 8 : 0 }} />
              {(parseFloat(extraAmt)||0) > 0 && (
                <div style={{ fontFamily:"var(--font-body)", color:"#FFA500", fontSize:12 }}>
          ...
          {status && status !== "delivered" && (
            <textarea value={note} onChange={function(e){ setNote(e.target.value); }} placeholder="Add a note (optional)..."
              style={{ width:"100%", background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,255,255,.15)", borderRadius:12, padding:"10px 14px", color:"#fff", fontFamily:"var(--font-body)", fontSize:14, minHeight:72, resize:"none", boxSizing:"border-box", marginBottom:16 }} />
          )}
        </div>
        ...
        <div style={{ flexShrink:0, background:"#0F1629", padding:"12px 20px", paddingBottom:"max(env(safe-area-inset-bottom, 0px) + 12px, 20px)", borderTop:"1px solid rgba(255,255,255,.06)" }}>
          <button onClick={handleConfirm} disabled={!canConfirm}
            style={{ width:"100%", background:canConfirm?"linear-gradient(135deg,#00D4FF,#7C3AED)":"rgba(255,255,255,.1)", border:"none", borderRadius:14, padding:15, color:"#fff", fontFamily:"var(--font-head)", fontWeight:700, fontSize:15, cursor:canConfirm?"pointer":"default" }}>
  ...
  return (
    <div style={{ background: earned ? "linear-gradient(135deg,rgba(16,185,129,.15),rgba(0,212,255,.1))" : "rgba(255,255,255,.04)", border:"1px solid " + (earned?"rgba(16,185,129,.3)":"rgba(255,255,255,.1)"), borderRadius:16, padding:16, marginBottom:14 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
        <div>
          <div style={{ fontFamily:"var(--font-head)", color: earned?"#10B981":"#fff", fontSize:14, fontWeight:700 }}>
            {earned ? "🎉 Commission Earned!" : " Commission Progress"}
          </div>
          <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.45)", fontSize:12, marginTop:2 }}>
            {earned
        ...
        {earned && (
          <div style={{ textAlign:"right" }}>
            <div style={{ fontFamily:"var(--font-head)", color:"#10B981", fontSize:22, fontWeight:800 }}>{fmt(amount)}</div>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11 }}>today&apos;s commission</div>
          </div>
        )}
      </div>
      {/* Progress bar */}
      ...
      <div style={{ display:"flex", justifyContent:"space-between" }}>
        <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11 }}>{deliveredCount} delivered</span>
        <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11 }}>
  ...
  return (
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"12px 0", borderBottom:"1px solid rgba(255,255,255,.06)" }}>
      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
        ...
        <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.45)", fontSize:13 }}>{label}</span>
      </div>
      <span style={{ fontFamily:"var(--font-head)", color: highlight || "#fff", fontSize:13, fontWeight:600, textAlign:"right", maxWidth:180 }}>{value || "-"}</span>
    </div>
  );
}
  ...
  return (
    <div style={{ flex:1, overflowY:"auto", padding:"0 16px 80px" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,rgba(0,212,255,.08),rgba(124,58,237,.1))", border:"1px solid rgba(0,212,255,.15)", borderRadius:20, padding:24, marginBottom:16, textAlign:"center" }}>
        <div style={{ width:72, height:72, borderRadius:"50%", background:"linear-gradient(135deg,#FF6B35,#FF3D71)", display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 12px", fontFamily:"var(--font-head)", fontSize:26, fontWeight:800, color:"#fff" }}>
          {driver.avatar || user.name?.slice(0,2).toUpperCase()}
        </div>
        <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:20, fontWeight:800 }}>{driver.name}</div>
        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:13, marginTop:4 }}>ID: {user.id.toUpperCase()}   Delivery Driver</div>
        <div style={{ display:"flex", justifyContent:"center", gap:8, marginTop:10 }}>
          <span style={{ background: driver.status==="active"?"rgba(16,185,129,.15)":"rgba(107,114,128,.15)", color: driver.status==="active"?"#10B981":"#9CA3AF", border:"1px solid " + (driver.status==="active"?"rgba(16,185,129,.3)":"rgba(107,114,128,.3)"), borderRadius:20, padding:"3px 12px", fontFamily:"var(--font-body)", fontSize:12, fontWeight:600 }}>
            {driver.status==="active" ? "Active" : "Inactive"}
          </span>
          <span style={{ background:"rgba(255,107,53,.1)", color:"#FF6B35", border:"1px solid rgba(255,107,53,.25)", borderRadius:20, padding:"3px 12px", fontFamily:"var(--font-body)", fontSize:12 }}>
      ...
      <div style={{ background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.08)", borderRadius:16, padding:"0 16px", marginBottom:14 }}>
        <div style={{ fontFamily:"var(--font-head)", color:"rgba(255,255,255,.35)", fontSize:10, fontWeight:700, letterSpacing:1.5, textTransform:"uppercase", padding:"12px 0 4px" }}>Personal Details</div>
      ...
      {/* Vehicle Details */}
      <div style={{ background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.08)", borderRadius:16, padding:"0 16px", marginBottom:14 }}>
        <div style={{ fontFamily:"var(--font-head)", color:"rgba(255,255,255,.35)", fontSize:10, fontWeight:700, letterSpacing:1.5, textTransform:"uppercase", padding:"12px 0 4px" }}>Vehicle Details</div>
      ...
      <div style={{ background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.08)", borderRadius:16, padding:"14px 16px", marginBottom:14 }}>
        <div style={{ fontFamily:"var(--font-head)", color:"rgba(255,255,255,.35)", fontSize:10, fontWeight:700, letterSpacing:1.5, textTransform:"uppercase", marginBottom:12 }}>Assigned Vehicle</div>
        {selVehicle ? (
          <div>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
              <div>
                <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:15, fontWeight:700 }}>{selVehicle.brand} {selVehicle.model}</div>
                <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12, marginTop:2 }}>
                  Plate: <span style={{ color:"#FF6B35", fontWeight:600 }}>{selVehicle.plate}</span>   {selVehicle.color} {selVehicle.type}
                </div>
              </div>
              <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.3)", fontSize:10 }}>Assigned by Admin</span>
            </div>
          </div>
        ) : (
          <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12 }}>No vehicle assigned yet. Contact admin.</div>
        )}
      </div>
      ...
      {(daftarWarning || daftarExpired) && (
        <div style={{ background: daftarExpired?"rgba(239,68,68,.1)":"rgba(245,158,11,.1)", border:"1px solid " + (daftarExpired?"rgba(239,68,68,.3)":"rgba(245,158,11,.3)"), borderRadius:12, padding:"12px 14px", marginBottom:14, display:"flex", gap:10, alignItems:"center" }}>
          ...
          <div>
            <div style={{ fontFamily:"var(--font-head)", color: daftarExpired?"#EF4444":"#F59E0B", fontSize:13, fontWeight:700 }}>
              {daftarExpired ? "Daftar EXPIRED!" : "Daftar expires in " + (daysToExpiry) + " days"}
            </div>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12 }}>Please renew vehicle registration immediately</div>
          </div>
        </div>
      )}
      ...
      {/* Today's Performance */}
      <div style={{ background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.08)", borderRadius:16, padding:"0 16px", marginBottom:14 }}>
        <div style={{ fontFamily:"var(--font-head)", color:"rgba(255,255,255,.35)", fontSize:10, fontWeight:700, letterSpacing:1.5, textTransform:"uppercase", padding:"12px 0 4px" }}>Today&apos;s Performance</div>
      ...
      {orders.length > 0 && (
        <div style={{ marginBottom:14 }}>
          <div style={{ fontFamily:"var(--font-head)", color:"rgba(255,255,255,.35)", fontSize:10, fontWeight:700, letterSpacing:1.5, textTransform:"uppercase", marginBottom:10 }}>Today's Order Activity</div>
          {[
            ...
            return (
              <div key={g.label} style={{ background:"rgba(255,255,255,.03)", border:"1px solid "+g.border, borderRadius:14, padding:"12px 14px", marginBottom:8 }}>
                <div style={{ fontFamily:"var(--font-head)", color:g.color, fontSize:13, fontWeight:700, marginBottom:6 }}>{g.label} — {g.items.length} order{g.items.length>1?"s":""}</div>
                {g.items.map(function(o){
                  return (
                    <div key={o.invoiceNo} style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", padding:"5px 0", borderBottom:"1px solid rgba(255,255,255,.04)" }}>
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.8)", fontSize:12, fontWeight:600 }}>{o.customer}</div>
                        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.3)", fontSize:10 }}>#{o.invoiceNo}{o.onlineOrderNo?" · OO:"+o.onlineOrderNo:""} · {o.store}</div>
                        {o.note?<div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.25)", fontSize:10, fontStyle:"italic" }}>{o.note}</div>:null}
                      </div>
                      <div style={{ textAlign:"right", marginLeft:8 }}>
                        <div style={{ fontFamily:"var(--font-head)", color:g.color, fontSize:12, fontWeight:700 }}>{fmt(o.total)}</div>
                        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.3)", fontSize:10 }}>{o.paymentType}</div>
                      </div>
                    </div>
                  );
                })}
                {g.label==="Delivered" && (
                  <div style={{ fontFamily:"var(--font-head)", color:g.color, fontSize:11, fontWeight:700, marginTop:6, textAlign:"right" }}>
  ...
  return (
    <div style={{ flex:1, overflowY:"auto", padding:"0 16px 80px" }}>
      ...
      <div style={{ background:"rgba(255,255,255,.04)", border:"1px solid rgba(255,255,255,.08)", borderRadius:16, padding:16, marginBottom:16 }}>
        <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:14, fontWeight:700, marginBottom:14 }}> Log Vehicle Expense</div>

        {/* Type selector */}
        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11, marginBottom:8 }}>EXPENSE TYPE</div>
        <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginBottom:14 }}>
          {EXPENSE_TYPES.map(t => (
            <button key={t} onClick={() => setExpType(t)}
              ...
              <span style={{ fontFamily:"var(--font-body)", color:expType===t?"#FF6B35":"rgba(255,255,255,.6)", fontSize:13, fontWeight:expType===t?600:400 }}>
        ...
        {/* Amount */}
        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11, marginBottom:6 }}>AMOUNT (KD)</div>
        <input type="number" value={amount} onChange={e=>setAmount(e.target.value)} placeholder="0.000" step="0.001" min="0"
          style={{ width:"100%", background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,255,255,.1)", borderRadius:12, padding:"12px 14px", color:"#FF6B35", fontFamily:"var(--font-head)", fontSize:16, fontWeight:700, outline:"none", boxSizing:"border-box", marginBottom:12 }} />

        {/* Note */}
        <input value={note} onChange={e=>setNote(e.target.value)} placeholder="Note (optional, e.g. filled at KNPC station)"
          style={{ width:"100%", background:"rgba(255,255,255,.06)", border:"1px solid rgba(255,255,255,.08)", borderRadius:12, padding:"11px 14px", color:"#fff", fontFamily:"var(--font-body)", fontSize:13, outline:"none", boxSizing:"border-box", marginBottom:12 }} />

        {err && <div style={{ fontFamily:"var(--font-body)", color:"#EF4444", fontSize:12, marginBottom:8 }}> {err}</div>}

        <button onClick={submit}
          style={{ width:"100%", background: saved?"#10B981":"linear-gradient(135deg,#FF6B35,#FF3D71)", border:"none", borderRadius:12, padding:14, color:"#fff", fontFamily:"var(--font-head)", fontWeight:700, fontSize:14, cursor:"pointer", transition:"background .3s" }}>
      ...
      {expenses.length > 0 && (
        <>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
            <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:14, fontWeight:700 }}>Today&apos;s Expenses</div>
            <div style={{ fontFamily:"var(--font-head)", color:"#EF4444", fontSize:15, fontWeight:800 }}>- {fmt(totalExp)}</div>
          </div>
          {expenses.map(function(e,i) {
            if (editId === e.id) {
              return (
                <div key={e.id||i} style={{ background:"rgba(255,107,53,.08)", border:"1px solid rgba(255,107,53,.3)", borderRadius:12, padding:"12px 14px", marginBottom:8 }}>
                  <div style={{ fontFamily:"var(--font-head)", color:"#FF6B35", fontSize:13, fontWeight:700, marginBottom:10 }}>Edit Expense</div>
                  <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:10 }}>
                    {EXPENSE_TYPES.map(function(t){ return (
                      <button key={t} onClick={function(){ setEditType(t); }}
                        ...
                        <span style={{ fontFamily:"var(--font-body)", color:editType===t?"#FF6B35":"rgba(255,255,255,.5)", fontSize:12 }}>{t}</span>
                      </button>
                    ); })}
                  </div>
                  <input type="number" value={editAmount} onChange={function(e){ setEditAmount(e.target.value); }} placeholder="Amount KD"
                    style={{ width:"100%", boxSizing:"border-box", background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,255,255,.1)", borderRadius:10, padding:"9px 12px", color:"#FF6B35", fontFamily:"var(--font-head)", fontSize:15, fontWeight:700, outline:"none", marginBottom:8 }} />
                  <input value={editNote} onChange={function(e){ setEditNote(e.target.value); }} placeholder="Note (optional)"
                    style={{ width:"100%", boxSizing:"border-box", background:"rgba(255,255,255,.06)", border:"1px solid rgba(255,255,255,.08)", borderRadius:10, padding:"9px 12px", color:"#fff", fontFamily:"var(--font-body)", fontSize:13, outline:"none", marginBottom:10 }} />
                  <div style={{ display:"flex", gap:8 }}>
                    <button onClick={function(){
                      ...
                      style={{ flex:1, background:"#FF6B35", border:"none", borderRadius:10, padding:"10px", color:"#fff", fontFamily:"var(--font-head)", fontWeight:700, fontSize:13, cursor:"pointer" }}>Save</button>
                    <button onClick={function(){ setEditId(null); }}
                      style={{ flex:1, background:"rgba(255,255,255,.08)", border:"none", borderRadius:10, padding:"10px", color:"rgba(255,255,255,.6)", fontFamily:"var(--font-body)", fontSize:13, cursor:"pointer" }}>Cancel</button>
                  </div>
                </div>
              );
            }
            return (
              <div key={e.id||i} style={{ background:"rgba(239,68,68,.05)", border:"1px solid rgba(239,68,68,.15)", borderRadius:12, padding:"12px 14px", marginBottom:8, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontFamily:"var(--font-body)", color:"#fff", fontSize:13, fontWeight:600 }}>{e.type==="Fuel"?"⛽":e.type==="Parking"?"🅿":e.type==="Toll"?"🛣":e.type==="Car Wash"?"🚿":e.type==="Maintenance"?"🔧":"📋"} {e.type}</div>
                  {e.note && <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11, marginTop:2 }}>{e.note}</div>}
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.25)", fontSize:10, marginTop:2 }}>{new Date(e.createdAt).toLocaleTimeString("en-KW",{hour:"2-digit",minute:"2-digit"})}</div>
                </div>
                <div style={{ display:"flex", alignItems:"center", gap:8, flexShrink:0 }}>
                  <div style={{ fontFamily:"var(--font-head)", color:"#EF4444", fontSize:14, fontWeight:800 }}>- {fmt(e.amount)}</div>
                  <button onClick={function(){ setEditId(e.id); setEditType(e.type); setEditAmount(String(e.amount)); setEditNote(e.note||""); }}
                    style={{ background:"rgba(255,255,255,.08)", border:"1px solid rgba(255,255,255,.15)", borderRadius:8, padding:"4px 8px", color:"rgba(255,255,255,.5)", fontFamily:"var(--font-body)", fontSize:11, cursor:"pointer" }}>Edit</button>
                  <button onClick={function(){ if(window.confirm("Delete this expense?")) { onDeleteExpense && onDeleteExpense(e.id); } }}
                    style={{ background:"rgba(239,68,68,.1)", border:"1px solid rgba(239,68,68,.25)", borderRadius:8, padding:"4px 8px", color:"#EF4444", fontFamily:"var(--font-body)", fontSize:11, cursor:"pointer" }}>✕</button>
                </div>
              </div>
            );
          })}
...
function SCard({ label, value, color, bg }) {
  return (
    <div style={{ background:bg, borderRadius:9, padding:"10px 12px", flex:1, minWidth:100 }}>
      <div style={{ fontFamily:"var(--font-head)", color:color, fontSize:20, fontWeight:800 }}>{value}</div>
      <div style={{ fontFamily:"var(--font-body)", color:"#475569", fontSize:11, marginTop:2 }}>{label}</div>
    </div>
  );
}
  ...
  return (
    <div style={{ position:"absolute", inset:0, zIndex:999, background:"#f8fafc", display:"flex", flexDirection:"column", overflowY:"auto" }}>
      ...
      <div style={{ position:"sticky", top:0, zIndex:10, background:"#0A0F1E", padding:"10px 14px", display:"flex", justifyContent:"space-between", alignItems:"center", flexShrink:0, flexWrap:"wrap", gap:8 }}>
        <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:13, fontWeight:700 }}>Daily Report - {drvName}</div>
        <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
          <button onClick={function() {
            ...
            ifr.src = url;
          }} style={{ background:"rgba(0,212,255,.15)", border:"1px solid rgba(0,212,255,.4)", borderRadius:8, padding:"6px 12px", color:"#00D4FF", fontFamily:"var(--font-head)", fontWeight:700, fontSize:12, cursor:"pointer" }}>
            Print A4
          </button>
          <button onClick={function() {
            var pri = document.getElementById("report-print-area");
          ...
          }} style={{ background:"rgba(16,185,129,.15)", border:"1px solid rgba(16,185,129,.4)", borderRadius:8, padding:"6px 12px", color:"#10B981", fontFamily:"var(--font-head)", fontWeight:700, fontSize:12, cursor:"pointer" }}>
            Download
          </button>
          <button onClick={onClose} style={{ background:"rgba(239,68,68,.2)", border:"1px solid rgba(239,68,68,.4)", borderRadius:8, padding:"6px 14px", color:"#EF4444", fontFamily:"var(--font-head)", fontWeight:700, fontSize:13, cursor:"pointer" }}>x Close</button>
        </div>
      </div>

      <div id="report-print-area" style={{ padding:"12px 14px 40px", fontFamily:"var(--font-body)", color:"#111", maxWidth:"100%", margin:"0 auto", width:"100%" }}>
        ...
        <div style={{ background:"linear-gradient(135deg,#0A0F1E,#1a2340)", color:"#fff", borderRadius:12, padding:"16px 18px", marginBottom:16, display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:8 }}>
          <div>
            <div style={{ fontFamily:"var(--font-head)", fontSize:18, fontWeight:800, letterSpacing:-1 }}>Deliver<span style={{ color:"#00D4FF" }}>Flow</span></div>
          ...
          <div style={{ textAlign:"right", fontSize:12, opacity:.75, lineHeight:1.8 }}>
            <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:14, fontWeight:700 }}>Delivery Boy: {drvName}</div>
            <div>Date: {date}</div>
        ...
        {/* Order Summary */}
        <div style={{ fontFamily:"var(--font-head)", fontSize:11, fontWeight:700, color:"#64748B", letterSpacing:1, textTransform:"uppercase", borderBottom:"2px solid #E2E8F0", paddingBottom:4, marginBottom:10 }}>Order Summary</div>
        ...
        {/* Collection Summary */}
        <div style={{ fontFamily:"var(--font-head)", fontSize:11, fontWeight:700, color:"#64748B", letterSpacing:1, textTransform:"uppercase", borderBottom:"2px solid #E2E8F0", paddingBottom:4, marginBottom:10, marginTop:16 }}>Collection Summary</div>
        <div style={{ background:"#fff", borderRadius:10, overflow:"hidden", border:"1px solid #E2E8F0", marginBottom:6 }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr auto auto", background:"#0A0F1E", padding:"7px 12px" }}>
            <span style={{ fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:11, fontWeight:600, textTransform:"uppercase" }}>Payment Method</span>
            <span style={{ fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:11, fontWeight:600, textTransform:"uppercase", minWidth:100, textAlign:"right" }}>Amount (KD)</span>
            <span style={{ fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:11, fontWeight:600, textTransform:"uppercase", minWidth:70, textAlign:"right" }}>Orders</span>
          </div>
          {collectionRows.map((r,i) => (
            <div key={r.label} style={{ display:"grid", gridTemplateColumns:"1fr auto auto", padding:"8px 12px", background:i%2===0?"#fff":"#F8FAFC", borderTop:"1px solid #F1F5F9" }}>
              <span style={{ fontFamily:"var(--font-body)", color:"#111", fontWeight:600, fontSize:13 }}>{r.label}</span>
              <span style={{ fontFamily:"var(--font-head)", color:"#111", fontWeight:700, fontSize:13, minWidth:100, textAlign:"right" }}>KD {r.amt.toFixed(3)}</span>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr auto auto", padding:"9px 12px", background:"#0A0F1E", borderTop:"2px solid #334155" }}>
            <span style={{ fontFamily:"var(--font-head)", color:"#fff", fontWeight:700, fontSize:13 }}>TOTAL COLLECTED</span>
            <span style={{ fontFamily:"var(--font-head)", color:"#00D4FF", fontWeight:800, fontSize:15, minWidth:100, textAlign:"right" }}>KD {grandTotal.toFixed(3)}</span>
            ...
            return (
              <div style={{ padding:"9px 12px", background:"#1e3a5f", borderTop:"1px solid #334155" }}>
                <div style={{ display:"grid", gridTemplateColumns:"1fr auto", marginBottom:4 }}>
                  <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.7)", fontSize:12 }}>Cash in Hand (COD)</span>
                  <span style={{ fontFamily:"var(--font-head)", color:"#F59E0B", fontWeight:700, fontSize:13 }}>KD {cashAmt.toFixed(3)}</span>
                </div>
                <div style={{ display:"grid", gridTemplateColumns:"1fr auto", marginBottom:4 }}>
                  <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.7)", fontSize:12 }}>Less: Vehicle Expenses</span>
                  <span style={{ fontFamily:"var(--font-head)", color:"#EF4444", fontWeight:700, fontSize:13 }}>- KD {totalExpAmt.toFixed(3)}</span>
                </div>
                <div style={{ display:"grid", gridTemplateColumns:"1fr auto", borderTop:"1px solid rgba(255,255,255,.2)", paddingTop:6 }}>
                  <span style={{ fontFamily:"var(--font-head)", color:"#fff", fontWeight:700, fontSize:13 }}>NET CASH TO RETURN</span>
                  <span style={{ fontFamily:"var(--font-head)", color:cashAfterExp>=0?"#10B981":"#EF4444", fontWeight:800, fontSize:15 }}>KD {cashAfterExp.toFixed(3)}</span>
                </div>
              </div>
            );
          })()}
        ...
        <div style={{ fontFamily:"var(--font-head)", fontSize:11, fontWeight:700, color:"#64748B", letterSpacing:1, textTransform:"uppercase", borderBottom:"2px solid #E2E8F0", paddingBottom:4, marginBottom:10 }}>Bill-wise Details</div>
        <div style={{ display:"flex", gap:8, background:"#0A0F1E", padding:"7px 10px", borderRadius:"10px 10px 0 0", marginBottom:2 }}>
          {["Date","Invoice No","Customer","OO No.","Total","Extra","Payment","Status","Note"].map(h=>(
            <span key={h} style={{ fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:10, fontWeight:600, textTransform:"uppercase", flex:h==="Customer"||h==="Note"?2:1, minWidth:0 }}>{h}</span>
          ))}
        </div>
          ...
          return (
            <div key={store} style={{ background:"#fff", border:"1px solid #E2E8F0", borderRadius:10, overflow:"hidden", marginBottom:10, pageBreakInside:"avoid" }}>
              ...
              <div style={{ background:"#1e293b", padding:"8px 10px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <span style={{ fontFamily:"var(--font-head)", color:"#00D4FF", fontWeight:700, fontSize:13 }}>{store}</span>
                <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:11 }}>
              ...
              {sDelOrds.length > 0 && (
                <div>
                  <div style={{ background:"#ECFDF5", padding:"4px 10px", fontFamily:"var(--font-head)", color:"#059669", fontSize:10, fontWeight:700, textTransform:"uppercase" }}>Delivered</div>
              ...
              {sExOrds.length > 0 && (
                <div>
                  <div style={{ background:"#F3F4F6", padding:"4px 10px", fontFamily:"var(--font-head)", color:"#6B7280", fontSize:10, fontWeight:700, textTransform:"uppercase" }}>Exchange (No Cash)</div>
              ...
              {sCancOrds.length > 0 && (
                <div>
                  <div style={{ background:"#FEF2F2", padding:"4px 10px", fontFamily:"var(--font-head)", color:"#DC2626", fontSize:10, fontWeight:700, textTransform:"uppercase" }}>Cancelled</div>
              ...
              {sPostOrds.length > 0 && (
                <div>
                  <div style={{ background:"#F5F3FF", padding:"4px 10px", fontFamily:"var(--font-head)", color:"#7C3AED", fontSize:10, fontWeight:700, textTransform:"uppercase" }}>Postponed</div>
              ...
              <div style={{ background:"#F8FAFC", padding:"8px 10px", borderTop:"1px solid #E2E8F0" }}>
                <div style={{ display:"flex", flexWrap:"wrap", gap:"4px 16px", marginBottom:4 }}>
                  {sCash>0  && <span style={{ fontFamily:"var(--font-body)", fontSize:11, color:"#059669" }}>Cash: <strong>KD {sCash.toFixed(3)}</strong></span>}
                  {sKnet>0  && <span style={{ fontFamily:"var(--font-body)", fontSize:11, color:"#0284C7" }}>KNET: <strong>KD {sKnet.toFixed(3)}</strong></span>}
                  {sDeema>0 && <span style={{ fontFamily:"var(--font-body)", fontSize:11, color:"#7C3AED" }}>Deema: <strong>KD {sDeema.toFixed(3)}</strong></span>}
                  {sTabby>0 && <span style={{ fontFamily:"var(--font-body)", fontSize:11, color:"#D97706" }}>Tabby: <strong>KD {sTabby.toFixed(3)}</strong></span>}
                  {sVisa>0  && <span style={{ fontFamily:"var(--font-body)", fontSize:11, color:"#DC2626" }}>VISA/MC: <strong>KD {sVisa.toFixed(3)}</strong></span>}
                  {sLink>0  && <span style={{ fontFamily:"var(--font-body)", fontSize:11, color:"#0891B2" }}>Link: <strong>KD {sLink.toFixed(3)}</strong></span>}
                  {sSplit>0 && <span style={{ fontFamily:"var(--font-body)", fontSize:11, color:"#6B7280" }}>Split: <strong>KD {sSplit.toFixed(3)}</strong></span>}
                  {sExtra>0 && <span style={{ fontFamily:"var(--font-body)", fontSize:11, color:"#EA580C" }}>⚡ Extra: <strong>KD {sExtra.toFixed(3)}</strong></span>}
                </div>
                <div style={{ fontFamily:"var(--font-head)", fontSize:12, fontWeight:700, color:"#0F172A", textAlign:"right" }}>
                  Store Total (Delivered): KD {sTotalWithExtra.toFixed(3)}
                  {sExtra>0 && <span style={{ fontFamily:"var(--font-body)", fontSize:10, color:"#EA580C", marginLeft:6, fontWeight:400 }}>(incl. KD {sExtra.toFixed(3)} extra)</span>}
                </div>
              </div>
            </div>
          );
        ...
        {/* Commission & Expenses */}
        <div style={{ fontFamily:"var(--font-head)", fontSize:11, fontWeight:700, color:"#64748B", letterSpacing:1, textTransform:"uppercase", borderBottom:"2px solid #E2E8F0", paddingBottom:4, marginBottom:10, marginTop:16 }}>Commission &amp; Expenses</div>
        ...
        {myExpenses.length > 0 && (
          <div style={{ background:"#fff", borderRadius:10, overflow:"hidden", border:"1px solid #E2E8F0" }}>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 2fr auto", background:"#0A0F1E", padding:"7px 12px" }}>
              {["Type","Amount","Note","Time"].map(h=><span key={h} style={{ fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:10, fontWeight:600 }}>{h}</span>)}
            </div>
            {myExpenses.map((e,i)=>(
...
function Stat({ l, v, c, icon }) {
  return (
    <div style={{ background:c+"10", border:"1px solid "+c+"25", borderRadius:14, padding:"14px 16px", display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
      <div>
        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:12 }}>{icon} {l}</div>
        <div style={{ fontFamily:"var(--font-head)", color:c, fontSize:22, fontWeight:800, marginTop:2 }}>{v}</div>
      </div>
    </div>
  );
}
  ...
  if (myOrders.length === 0) {
    return (
      <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", gap:12, padding:32 }}>
        ...
        <div style={{ fontFamily:"var(--font-head)", color:"rgba(255,255,255,.4)", fontSize:16 }}>No orders to close today</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ flex:1, overflowY:"auto", padding:"16px 16px 80px" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,rgba(16,185,129,.12),rgba(0,212,255,.08))", border:"1px solid rgba(16,185,129,.25)", borderRadius:16, padding:"16px 18px", marginBottom:16 }}>
        <div style={{ fontFamily:"var(--font-head)", color:"#10B981", fontSize:16, fontWeight:800, marginBottom:4 }}>Day Closing</div>
        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:12 }}>
          Send closing reports to each store via WhatsApp
        </div>
        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:11, marginTop:4 }}>
        ...
        {[
          ...
          return (
            <div key={s.label} style={{ background:"rgba(255,255,255,.04)", border:"1px solid rgba(255,255,255,.08)", borderRadius:12, padding:"10px 8px", textAlign:"center" }}>
              <div style={{ fontFamily:"var(--font-head)", color:s.color, fontSize:20, fontWeight:800 }}>{s.value}</div>
              <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:10, marginTop:2 }}>{s.label}</div>
            </div>
          );
        })}
      </div>
      ...
      {stores.map(function(store) {
        ...
        return (
          <div key={store} style={{ background:"rgba(255,255,255,.04)", border:"1px solid rgba(255,255,255,.08)", borderRadius:16, padding:"16px", marginBottom:12 }}>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
              <div>
                <div style={{ fontFamily:"var(--font-head)", color:storeColor, fontSize:14, fontWeight:700 }}>{store}</div>
                <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:11, marginTop:2 }}>WA: +{waNum}</div>
              </div>
              {isSent && (
                <span style={{ background:"rgba(16,185,129,.15)", border:"1px solid rgba(16,185,129,.3)", borderRadius:20, padding:"2px 10px", fontFamily:"var(--font-body)", color:"#10B981", fontSize:11 }}>
                  ✓ Sent
                </span>
              )}
            </div>
              ...
              {[
                ...
                return (
                  <div key={s.label} style={{ background:"rgba(255,255,255,.03)", borderRadius:10, padding:"8px 4px", textAlign:"center" }}>
                    <div style={{ fontFamily:"var(--font-head)", color:s.color, fontSize:s.small?11:16, fontWeight:700, lineHeight:1.2 }}>{s.value}</div>
                    <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.3)", fontSize:9, marginTop:2 }}>{s.label}</div>
                  </div>
                );
              })}
            </div>
            ...
            <div style={{ background:"rgba(255,255,255,.03)", borderRadius:10, padding:"10px 12px", marginBottom:12 }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11 }}>💵 Cash COD</span>
                <span style={{ fontFamily:"var(--font-head)", color:"#F59E0B", fontSize:12, fontWeight:700 }}>KD {stats.cashAmt.toFixed(3)}</span>
              </div>
              <div style={{ display:"flex", justifyContent:"space-between" }}>
                <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11 }}>💳 Online/Link</span>
                <span style={{ fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:12, fontWeight:700 }}>KD {stats.onlineAmt.toFixed(3)}</span>
              </div>
            </div>
            ...
            <a href={getWAUrl(store, stats) || "#"}
              ...
              <span style={{ fontSize:18 }}>💬</span>
              <span style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:13, fontWeight:700 }}>
      ...
      {stores.length > 1 && (
        <button onClick={sendAllWhatsApp}
          ...
          <span style={{ fontSize:18 }}>📤</span>
          <span style={{ fontFamily:"var(--font-head)", color:"#25D366", fontSize:13, fontWeight:700 }}>Send All Reports</span>
        </button>
      )}
    </div>
  );
  ...
  return (
    <div style={{ flex:1, overflowY:"auto", padding:"0 16px 80px" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,rgba(0,212,255,.1),rgba(124,58,237,.1))", border:"1px solid rgba(0,212,255,.2)", borderRadius:20, padding:20, marginBottom:14, textAlign:"center" }}>
        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:13 }}>Today&apos;s Summary   {new Date().toLocaleDateString("en-KW")}</div>
        <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:46, fontWeight:800, lineHeight:1 }}>{myOrders.length}</div>
        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:13, marginTop:4 }}>Total Orders Assigned</div>
        ...
        <div style={{ fontFamily:"var(--font-body)", color:"#10B981", fontSize:12, marginTop:4 }}>{rate}% success rate</div>
      </div>
      ...
      {(expenses||[]).length > 0 && (
        <div style={{ background:"rgba(239,68,68,.06)", border:"1px solid rgba(239,68,68,.2)", borderRadius:14, padding:"14px 16px", marginBottom:10 }}>
          <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:12 }}> Vehicle Expenses</div>
          <div style={{ fontFamily:"var(--font-head)", color:"#EF4444", fontSize:22, fontWeight:800, marginTop:2 }}>- {fmt(totalExpenses)}</div>
          <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginTop:8 }}>
            {EXPENSE_TYPES.map(function(expT) {
              const amt = (expenses||[]).filter(e=>e.type===expT).reduce((a,e)=>a+Number(e.amount),0);
              if (!amt) return null;
              return <span key={expT} style={{ background:"rgba(239,68,68,.1)", borderRadius:20, padding:"2px 9px", fontFamily:"var(--font-body)", color:"#FCA5A5", fontSize:11 }}>{expT}: {fmt(amt)}</span>;
            })}
          </div>
        </div>
      )}
      ...
      <div style={{ background:"rgba(107,114,128,.08)", border:"1px solid rgba(107,114,128,.25)", borderRadius:16, marginBottom:14, overflow:"hidden" }}>
        <button onClick={() => setShowExchangeList(e => !e)}
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <span style={{ fontSize:20 }}></span>
            <div style={{ textAlign:"left" }}>
              <div style={{ fontFamily:"var(--font-head)", color:"#9CA3AF", fontSize:14, fontWeight:700 }}>Exchange Orders</div>
              <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12 }}>No cash collection - deliver &amp; exchange only</div>
            </div>
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <span style={{ fontFamily:"var(--font-head)", color:"#9CA3AF", fontSize:22, fontWeight:800 }}>{exchangeOrders.length}</span>
        ...
        {showExchangeList && (
          <div style={{ borderTop:"1px solid rgba(255,255,255,.06)", padding:"12px 14px" }}>
            {exchangeOrders.length === 0 ? (
              <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.3)", fontSize:13, textAlign:"center", padding:"12px 0" }}>No exchange orders today</div>
              ...
              <div key={o.id||i} style={{ background:"rgba(255,255,255,.04)", borderRadius:10, padding:"10px 12px", marginBottom:8, display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontFamily:"var(--font-body)", color:"#fff", fontSize:13, fontWeight:600 }}>{o.customer}</div>
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:11 }}>#{o.invoiceNo}{o.onlineOrderNo?" · Online Order: "+o.onlineOrderNo:""} · {o.store}</div>
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:11, marginTop:2 }}> {o.address?.slice(0,50)}{o.address?.length>50?"...":""}</div>
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:11 }}> {o.phone}</div>
                </div>
                <div style={{ textAlign:"right", flexShrink:0, marginLeft:10 }}>
                  <div style={{ fontFamily:"var(--font-head)", color:"#9CA3AF", fontSize:13, fontWeight:700 }}>{fmt(o.total)}</div>
                  <span style={{ background:"rgba(107,114,128,.2)", color:"#9CA3AF", borderRadius:20, padding:"2px 8px", fontSize:10, fontFamily:"var(--font-body)", fontWeight:600 }}> Exchange</span>
      ...
      <button onClick={() => onOpenReport && onOpenReport(buildReportData())}
        style={{ width:"100%", background:"linear-gradient(135deg,#FF6B35,#FF3D71)", border:"none", borderRadius:14, padding:16, color:"#fff", fontFamily:"var(--font-head)", fontWeight:700, fontSize:15, cursor:"pointer", marginBottom:8, display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
        <span style={{ fontSize:18 }}></span> Preview &amp; Share Report
      </button>
      <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:12, textAlign:"center", marginBottom:16 }}>
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#0A0F1E", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24 }}>
      <div style={{ marginBottom:32, textAlign:"center" }}>
        ...
        <div style={{ fontFamily:"var(--font-head)", fontSize:28, fontWeight:800, color:"#fff", letterSpacing:-1 }}>Deliver<span style={{ color:"#00D4FF" }}>Flow</span></div>
        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:13, marginTop:4 }}>AMTEL TELECOM &nbsp; Warehouse System</div>
      </div>
      <div style={{ width:"100%", maxWidth:360, background:"rgba(255,255,255,.05)", borderRadius:20, padding:28, border:"1px solid rgba(255,255,255,.08)" }}>
        <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:18, fontWeight:700, marginBottom:20 }}>Sign In</div>
        <input type="text" placeholder="Username" value={user}
          ...
          style={{ width:"100%", background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,255,255,.1)", borderRadius:12, padding:"12px 16px", color:"#fff", fontFamily:"var(--font-body)", fontSize:15, marginBottom:12, boxSizing:"border-box", outline:"none" }} />
        <input type="password" placeholder="Password" value={pass}
          onChange={function(e) { setPass(e.target.value); }}
          onKeyDown={function(e) { if (e.key === "Enter") performLogin(user, pass, setErr, onLogin); }}
          style={{ width:"100%", background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,255,255,.1)", borderRadius:12, padding:"12px 16px", color:"#fff", fontFamily:"var(--font-body)", fontSize:15, marginBottom:12, boxSizing:"border-box", outline:"none" }} />
        {err && <div style={{ color:"#EF4444", fontFamily:"var(--font-body)", fontSize:13, marginBottom:10 }}>{err}</div>}
        <button onClick={function() { performLogin(user, pass, setErr, onLogin); }}
          style={{ width:"100%", background:"linear-gradient(135deg,#FF6B35,#FF3D71)", border:"none", borderRadius:12, padding:14, color:"#fff", fontFamily:"var(--font-head)", fontSize:16, fontWeight:700, cursor:"pointer" }}>
          Sign In
        </button>
        <div style={{ textAlign:"center", marginTop:20, fontFamily:"var(--font-body)", color:"rgba(255,255,255,.2)", fontSize:11 }}>
...
function AdminHistoryTab({ history }) {
  ...
  if (!history || history.length === 0) {
    return (
      <div style={{ padding:"40px 24px", textAlign:"center" }}>
        ...
        <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:16, fontWeight:700, marginBottom:8 }}>No History Yet</div>
        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:13, lineHeight:1.6 }}>
  ...
  return (
    <div style={{ padding:"0 16px 80px" }}>
      <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:15, fontWeight:700, marginBottom:4 }}>Driver History</div>
      <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12, marginBottom:16 }}>Daily performance records — last {history.length} day{history.length!==1?"s":""}</div>
        ...
        {history.map(function(h) {
          ...
          return (
            <button key={h.timestamp} onClick={function() { setSelectedDay(h); setExpandedDriver(null); }}
              ...
              <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:12, fontWeight:700 }}>{h.date}</div>
              <div style={{ fontFamily:"var(--font-body)", color:isSelected?"rgba(255,255,255,.7)":"rgba(255,255,255,.35)", fontSize:10, marginTop:2 }}>{h.drivers.length} drivers</div>
            </button>
          );
        })}
      </div>
          ...
          return [
            ...
            return (
              <div key={item[1]} style={{ background:item[2]+"12", border:"1px solid "+item[2]+"25", borderRadius:12, padding:"10px 8px", textAlign:"center" }}>
                <div style={{ fontFamily:"var(--font-head)", color:item[2], fontSize:16, fontWeight:800 }}>{item[0]}</div>
                <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:10, marginTop:2 }}>{item[1]}</div>
              </div>
            );
          });
        })()}
        ...
        return (
          <div key={d.id} style={{ background:"rgba(255,255,255,.04)", border:"1px solid rgba(255,255,255,.08)", borderRadius:16, marginBottom:10, overflow:"hidden" }}>
            ...
            <div onClick={function() { setExpandedDriver(isExp ? null : d.id); }}
              ...
              <div style={{ width:40, height:40, borderRadius:"50%", background:"linear-gradient(135deg,#FF6B35,#FF3D71)", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"var(--font-head)", fontSize:14, fontWeight:800, color:"#fff", flexShrink:0 }}>{d.avatar}</div>
              <div style={{ flex:1 }}>
                <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:14, fontWeight:700 }}>{d.name}</div>
                <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11, marginTop:2 }}>
              ...
              <div style={{ textAlign:"right" }}>
                <div style={{ fontFamily:"var(--font-head)", color:"#10B981", fontSize:13, fontWeight:700 }}>{fmt(d.cashCollected)}</div>
                <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.3)", fontSize:10 }}>Cash collected</div>
              </div>
            ...
            {isExp && (
              <div style={{ borderTop:"1px solid rgba(255,255,255,.06)", padding:"12px 14px", background:"rgba(0,0,0,.15)" }}>
                  ...
                  {[
                    ...
                    return (
                      <div key={row[0]} style={{ background:"rgba(255,255,255,.04)", borderRadius:10, padding:"8px 10px" }}>
                        <div style={{ fontFamily:"var(--font-head)", color:row[2], fontSize:16, fontWeight:800 }}>{row[1]}</div>
                        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:10, marginTop:2 }}>{row[0]}</div>
                      </div>
                    );
                  })}
                </div>
                ...
                <div style={{ background:"rgba(255,255,255,.04)", borderRadius:12, padding:"12px 14px", marginBottom:8 }}>
                  <div style={{ fontFamily:"var(--font-head)", color:"rgba(255,255,255,.5)", fontSize:10, textTransform:"uppercase", letterSpacing:1, marginBottom:8 }}>Financial Summary</div>
                  {[
                    ...
                    return (
                      <div key={row[0]} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
                        <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:12 }}>{row[0]}</span>
                        <span style={{ fontFamily:"var(--font-head)", color:row[2], fontSize:13, fontWeight:700 }}>{row[1]}</span>
                      </div>
                    );
                  })}
                </div>
                ...
                {d.vehicleExpenses > 0 && (
                  <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                    {Object.keys(d.expenseBreakdown).map(function(type) {
                      return (
                        <span key={type} style={{ background:"rgba(239,68,68,.1)", border:"1px solid rgba(239,68,68,.2)", borderRadius:20, padding:"3px 10px", fontFamily:"var(--font-body)", color:"#FCA5A5", fontSize:11 }}>
  ...
  return (
    <div style={{ marginTop:10, marginBottom:4 }}>
      <button onClick={function(){ setOpen(function(p){return !p;}); setSelected(new Set()); }}
        ...
        <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.6)", fontSize:12 }}>Manage Orders ({dOrders.length})</span>
      ...
      {open && (
        <div style={{ background:"rgba(0,0,0,.2)", borderRadius:"0 0 10px 10px", padding:"10px 12px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
            <button onClick={toggleAll} style={{ background:"none", border:"1px solid rgba(255,255,255,.15)", borderRadius:6, padding:"3px 10px", color:"rgba(255,255,255,.5)", fontFamily:"var(--font-body)", fontSize:11, cursor:"pointer" }}>
              {selected.size === dOrders.length ? "Deselect All" : "Select All"}
            </button>
            {selected.size > 0 && (
              <button onClick={removeSelected}
                style={{ background:"rgba(239,68,68,.15)", border:"1px solid rgba(239,68,68,.3)", borderRadius:6, padding:"3px 10px", color:"#EF4444", fontFamily:"var(--font-body)", fontSize:11, fontWeight:600, cursor:"pointer" }}>
                Remove ({selected.size})
              </button>
            )}
          </div>
            ...
            {dOrders.map(function(o) {
              ...
              return (
                <div key={oid} onClick={function(){ setSelected(function(prev){ var n=new Set(prev); if(n.has(oid)){n.delete(oid);}else{n.add(oid);} return n; }); }}
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontFamily:"var(--font-body)", color:"#fff", fontSize:11, fontWeight:500, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{o.customer}</div>
                    <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.3)", fontSize:10 }}>#{o.invoiceNo} - {fmt(o.total)}</div>
                  </div>
                  <span style={{ background:sc.bg, color:sc.color, borderRadius:10, padding:"1px 7px", fontSize:9, fontFamily:"var(--font-body)", fontWeight:600, flexShrink:0 }}>{sc.label}</span>
                </div>
              );
            })}
          </div>
...
function AdminVehiclesTab({ orders, expenses, driverProfiles, onUpdateDriver, onAddDriver, passwords, onSetPassword, onRemoveOrderAdmin, onlineDrivers, activeDrivers }) {
  ...
  return (
    <div style={{ padding:"0 16px 80px" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
        <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:15, fontWeight:700 }}>Drivers &amp; Expenses</div>
        <button onClick={function(){ setShowAdd(true); }} style={{ background:"linear-gradient(135deg,#FF6B35,#FF3D71)", border:"none", borderRadius:10, padding:"7px 14px", color:"#fff", fontFamily:"var(--font-head)", fontSize:12, fontWeight:700, cursor:"pointer" }}>+ Add Driver</button>
      </div>
      <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12, marginBottom:14 }}>Manage drivers, view commission &amp; expenses</div>
      ...
      <div style={{ background:"rgba(0,212,255,.05)", border:"1px solid rgba(0,212,255,.2)", borderRadius:14, padding:"14px", marginBottom:16 }}>
        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6 }}>
          <span style={{ fontSize:18 }}>🔑</span>
          <div style={{ fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:13, fontWeight:700 }}>Label Scan API Key</div>
          {savedKey && <span style={{ background:"rgba(16,185,129,.15)", border:"1px solid rgba(16,185,129,.3)", borderRadius:20, padding:"2px 8px", fontFamily:"var(--font-body)", color:"#10B981", fontSize:10, fontWeight:600 }}>✓ Set</span>}
        </div>
        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11, marginBottom:10 }}>
        ...
        <div style={{ display:"flex", gap:8 }}>
          ...
          <button onClick={function(){ setShowApiKey(function(v){ return !v; }); }}
            style={{ background:"rgba(255,255,255,.08)", border:"1px solid rgba(255,255,255,.12)", borderRadius:10, padding:"9px 12px", color:"rgba(255,255,255,.5)", fontFamily:"var(--font-body)", fontSize:11, cursor:"pointer", flexShrink:0 }}>
            {showApiKey ? "Hide" : "Show"}
          </button>
          <button onClick={handleSaveApiKey}
            style={{ background: apiKeySaved ? "rgba(16,185,129,.2)" : "rgba(0,212,255,.15)", border: apiKeySaved ? "1px solid #10B981" : "1px solid rgba(0,212,255,.4)", borderRadius:10, padding:"9px 14px", color: apiKeySaved ? "#10B981" : "#00D4FF", fontFamily:"var(--font-body)", fontSize:12, fontWeight:700, cursor:"pointer", flexShrink:0 }}>
      ...
      <div style={{ background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.07)", borderRadius:14, padding:"12px 14px", marginBottom:16 }}>
        <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:13, fontWeight:700, marginBottom:10 }}>Login Credentials</div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6, marginBottom:8 }}>
          <div style={{ background:"rgba(255,107,53,.08)", borderRadius:10, padding:"8px 10px" }}>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:10, marginBottom:2 }}>ADMIN</div>
          ...
          {allDrivers.slice(0,3).map(function(d) {
            return (
              <div key={d.id} style={{ background:"rgba(0,212,255,.06)", borderRadius:10, padding:"8px 10px" }}>
                <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:10, marginBottom:2 }}>{d.name.toUpperCase()}</div>
        ...
        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.25)", fontSize:11 }}>To change a password: tap Edit on the driver card below</div>
      </div>
      ...
      {showAdd && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.85)", zIndex:300, display:"flex", alignItems:"flex-end" }}>
          <div style={{ width:"100%", background:"#0F1629", borderRadius:"20px 20px 0 0", padding:20, maxHeight:"85vh", overflowY:"auto" }}>
            <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:16, fontWeight:700, marginBottom:16 }}>Add New Driver</div>
            {[["id","Driver ID (login username)"],["name","Full Name"],["avatar","Avatar (2 letters, e.g. AS)"],["phone","Phone Number"],["nationality","Nationality"],["vehicleType","Vehicle Type (Car/Van/Bike)"],["vehicleNo","Vehicle Number"],["licenseNo","License No"],["joinDate","Join Date (YYYY-MM-DD)"],["daftarExpiry","Daftar Expiry (YYYY-MM-DD)"]].map(function(f){
              return (
                <div key={f[0]} style={{ marginBottom:10 }}>
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:11, marginBottom:4 }}>{f[1]}</div>
                  <input value={newDrv[f[0]]||""} onChange={function(e){ var v=e.target.value; setNewDrv(function(p){ var n={...p}; n[f[0]]=v; return n; }); }}
                    style={{ width:"100%", background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,255,255,.15)", borderRadius:10, padding:"10px 12px", color:"#fff", fontFamily:"var(--font-body)", fontSize:13, boxSizing:"border-box" }} />
                </div>
              );
            })}
            <div style={{ display:"flex", gap:10, marginTop:16 }}>
              <button onClick={function(){ setShowAdd(false); setNewDrv({ id:"", name:"", avatar:"", phone:"", status:"active", vehicleType:"Car", vehicleNo:"", licenseNo:"", nationality:"", joinDate:"", daftarExpiry:"" }); }}
                style={{ flex:1, background:"rgba(255,255,255,.08)", border:"none", borderRadius:12, padding:13, color:"#fff", fontFamily:"var(--font-head)", fontWeight:700, fontSize:14, cursor:"pointer" }}>Cancel</button>
              <button onClick={function(){
              ...
              }} style={{ flex:2, background:"linear-gradient(135deg,#00D4FF,#7C3AED)", border:"none", borderRadius:12, padding:13, color:"#fff", fontFamily:"var(--font-head)", fontWeight:700, fontSize:14, cursor:"pointer" }}>Save Driver</button>
            </div>
          </div>
        </div>
      )}
      ...
      {editDriver && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.85)", zIndex:300, display:"flex", alignItems:"flex-end" }}>
          <div style={{ width:"100%", background:"#0F1629", borderRadius:"20px 20px 0 0", padding:20, maxHeight:"85vh", overflowY:"auto" }}>
            <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:16, fontWeight:700, marginBottom:16 }}>Edit: {editDriver.name}</div>
            {/* Password change */}
            <div style={{ marginBottom:14, background:"rgba(0,212,255,.06)", border:"1px solid rgba(0,212,255,.15)", borderRadius:12, padding:"12px 14px" }}>
              <div style={{ fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:13, fontWeight:700, marginBottom:8 }}>Change Password</div>
              <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11, marginBottom:8 }}>
              ...
              <input placeholder="New password" id={"pwd-"+editDriver.id}
                style={{ width:"100%", background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,255,255,.15)", borderRadius:10, padding:"9px 12px", color:"#fff", fontFamily:"var(--font-body)", fontSize:13, boxSizing:"border-box" }} />
              <button onClick={function(){
              ...
              }} style={{ marginTop:8, background:"rgba(0,212,255,.15)", border:"1px solid rgba(0,212,255,.3)", borderRadius:8, padding:"7px 16px", color:"#00D4FF", fontFamily:"var(--font-head)", fontSize:12, fontWeight:700, cursor:"pointer" }}>
                Update Password
              </button>
            </div>
            ...
            <div style={{ marginBottom:14 }}>
              <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:12, marginBottom:6 }}>Assign Company Vehicle</div>
              {(function(){
                var curV = null;
                ...
                return (
                  <div style={{ display:"flex", gap:8 }}>
                    <select
                      ...
                      style={{ flex:1, background:"rgba(255,255,255,.07)", border:"1px solid rgba(255,255,255,.15)", borderRadius:10, padding:"10px 12px", color:"#fff", fontFamily:"var(--font-body)", fontSize:13, outline:"none", cursor:"pointer" }}>
            ...
            <div style={{ display:"flex", gap:10, marginTop:16 }}>
              <button onClick={function(){ setEditDriver(null); }} style={{ flex:1, background:"rgba(255,255,255,.08)", border:"none", borderRadius:12, padding:13, color:"#fff", fontFamily:"var(--font-head)", fontWeight:700, fontSize:14, cursor:"pointer" }}>Cancel</button>
              <button onClick={function(){
                if (onUpdateDriver) onUpdateDriver(editDriver.id, editDriver);
                setEditDriver(null);
              }} style={{ flex:2, background:"linear-gradient(135deg,#00D4FF,#7C3AED)", border:"none", borderRadius:12, padding:13, color:"#fff", fontFamily:"var(--font-head)", fontWeight:700, fontSize:14, cursor:"pointer" }}>Save Changes</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:16 }}>
        <div style={{ background:"rgba(16,185,129,.1)", border:"1px solid rgba(16,185,129,.25)", borderRadius:14, padding:14, textAlign:"center" }}>
          <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:11 }}> Total Commission</div>
          <div style={{ fontFamily:"var(--font-head)", color:"#10B981", fontSize:20, fontWeight:800, marginTop:4 }}>
        ...
        <div style={{ background:"rgba(239,68,68,.1)", border:"1px solid rgba(239,68,68,.25)", borderRadius:14, padding:14, textAlign:"center" }}>
          <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:11 }}> Total Expenses</div>
          <div style={{ fontFamily:"var(--font-head)", color:"#EF4444", fontSize:20, fontWeight:800, marginTop:4 }}>{fmt(totalExpenses)}</div>
        </div>
      </div>
          ...
          <div key={d.id} style={{ background:"rgba(255,255,255,.04)", border:"1px solid " + (daftarExp?"rgba(239,68,68,.4)":daftarWarn?"rgba(245,158,11,.3)":"rgba(255,255,255,.08)"), borderRadius:16, padding:16, marginBottom:14 }}>
            ...
            {(daftarExp || daftarWarn) && (
              <div style={{ background:daftarExp?"rgba(239,68,68,.12)":"rgba(245,158,11,.1)", borderRadius:10, padding:"8px 12px", marginBottom:12, display:"flex", gap:8, alignItems:"center" }}>
                ...
                <div>
                  <div style={{ fontFamily:"var(--font-head)", color:daftarExp?"#EF4444":"#F59E0B", fontSize:12, fontWeight:700 }}>{daftarExp?"Daftar EXPIRED":"Daftar expires in " + (daysToExpiry) + " days"}</div>
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11 }}>Due: {daftarDate?.toLocaleDateString("en-KW",{day:"numeric",month:"short",year:"numeric"})}</div>
                </div>
              </div>
            )}
            ...
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
              <div style={{ position:"relative", flexShrink:0 }}>
                <div style={{ width:46, height:46, borderRadius:"50%", background:"linear-gradient(135deg,#FF6B35,#FF3D71)", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"var(--font-head)", fontSize:16, fontWeight:800, color:"#fff" }}>{d.avatar}</div>
              ...
              <div style={{ flex:1 }}>
                <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:15, fontWeight:700 }}>{d.name}</div>
                <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12 }}>{d.phone}   {d.nationality}</div>
              </div>
              <span style={{ background:d.status==="active"?"rgba(16,185,129,.15)":"rgba(107,114,128,.15)", color:d.status==="active"?"#10B981":"#9CA3AF", borderRadius:20, padding:"3px 10px", fontFamily:"var(--font-body)", fontSize:11, fontWeight:600 }}>
                {d.status==="active"?"Active":"Inactive"}
              </span>
              <button onClick={function(){ setEditDriver({...d}); }}
                style={{ background:"rgba(0,212,255,.15)", border:"1px solid rgba(0,212,255,.35)", borderRadius:8, padding:"4px 12px", color:"#00D4FF", fontFamily:"var(--font-head)", fontSize:11, fontWeight:700, cursor:"pointer", flexShrink:0 }}>Edit</button>
            </div>
            ...
            <div style={{ background:"rgba(255,255,255,.04)", borderRadius:10, padding:"10px 12px", marginBottom:10, display:"flex", flexWrap:"wrap", gap:14 }}>
              <div><div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:10 }}>VEHICLE</div><div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:12, fontWeight:600 }}>{vehicleIcon} {d.vehicleType}   {d.vehicleNo}</div></div>
              <div><div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:10 }}>LICENSE NO.</div><div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.7)", fontSize:12 }}>{d.licenseNo}</div></div>
              <div><div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:10 }}>DAFTAR RENEWAL</div>
                <div style={{ fontFamily:"var(--font-head)", color:daftarExp?"#EF4444":daftarWarn?"#F59E0B":"#10B981", fontSize:12, fontWeight:600 }}>
                  {daftarDate ? daftarDate.toLocaleDateString("en-KW",{day:"numeric",month:"short",year:"numeric"}) : "-"}
                  {daysToExpiry !== null && <span style={{ fontFamily:"var(--font-body)", fontSize:11, fontWeight:400 }}> ({daftarExp?"Expired":(daysToExpiry) + "d left"})</span>}
                </div>
              </div>
              <div><div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:10 }}>JOINED</div><div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.6)", fontSize:12 }}>{d.joinDate ? new Date(d.joinDate).toLocaleDateString("en-KW",{month:"short",year:"numeric"}) : "-"}</div></div>
            </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:8, marginBottom:10 }}>
              {[[dOrders.length,"Assigned","#fff"],[scanned,"Collected","#00D4FF"],[done,"Delivered","#10B981"],[fmt(cash),"Cash COD","#F59E0B"]].map(function(item) { const v=item[0],l=item[1],c=item[2]; return (
                <div key={l} style={{ textAlign:"center", background:"rgba(255,255,255,.04)", borderRadius:10, padding:"8px 4px" }}>
                  <div style={{ fontFamily:"var(--font-head)", color:c, fontSize:l==="Cash COD"?10:15, fontWeight:800 }}>{v}</div>
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:10 }}>{l}</div>
                </div>
              )})}
            </div>
            ...
            <div style={{ background:"rgba(255,255,255,.04)", borderRadius:10, padding:"10px 12px", marginBottom:dExp.length?10:0 }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:5 }}>
                <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:12 }}> Commission {comm.earned?"Earned":"Progress"} <span style={{ color:"rgba(255,255,255,.25)", fontSize:10 }}>(excl. exchange)</span></span>
                <span style={{ fontFamily:"var(--font-head)", color:comm.earned?"#10B981":"#F59E0B", fontSize:13, fontWeight:700 }}>{comm.earned ? fmt(comm.amount) : (doneNoEx) + "/" + (COMMISSION_THRESHOLD)}</span>
              </div>
              ...
              {!comm.earned && <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.3)", fontSize:10, marginTop:3 }}>Need {Math.max(0,COMMISSION_THRESHOLD+1-doneNoEx)} more   KD {COMMISSION_PER_ORDER.toFixed(3)}/order from 21st onwards</div>}
            </div>
            ...
            {dExp.length > 0 && (
              <div style={{ borderTop:"1px solid rgba(255,255,255,.06)", paddingTop:10 }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                  <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12 }}> Vehicle Expenses So Far</span>
                  <span style={{ fontFamily:"var(--font-head)", color:"#EF4444", fontSize:13, fontWeight:700 }}>- {fmt(dExpAmt)}</span>
                </div>
                <div style={{ display:"flex", flexWrap:"wrap", gap:5 }}>
                  {EXPENSE_TYPES.map(function(expT) { const amt = dExp.filter(e=>e.type===expT).reduce((a,e)=>a+Number(e.amount),0); if(!amt)return null; return <span key={expT} style={{ background:"rgba(239,68,68,.1)", borderRadius:20, padding:"2px 9px", fontFamily:"var(--font-body)", color:"#FCA5A5", fontSize:11 }}>{expT}: {fmt(amt)}</span>; })}
                </div>
              </div>
            )}
          </div>
      ...
      {expenses.length > 0 && (
        <div style={{ marginTop:8 }}>
          <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:14, fontWeight:700, marginBottom:10 }}>All Expense Records</div>
          {expenses.slice().reverse().map((e,i) => (
            <div key={e.id||i} style={{ background:"rgba(239,68,68,.04)", border:"1px solid rgba(239,68,68,.12)", borderRadius:12, padding:"10px 14px", marginBottom:7, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div>
                <div style={{ fontFamily:"var(--font-body)", color:"#fff", fontSize:13, fontWeight:600 }}>{e.type} - {DRIVERS.find(d=>d.id===e.driverId)?.name}</div>
                {e.note && <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11 }}>{e.note}</div>}
                <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.25)", fontSize:10 }}>{new Date(e.createdAt).toLocaleString("en-KW",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}</div>
              </div>
              <div style={{ fontFamily:"var(--font-head)", color:"#EF4444", fontSize:14, fontWeight:800 }}>- {fmt(e.amount)}</div>
            </div>
          ))}
      ...
      {expenses.length === 0 && (
        <div style={{ textAlign:"center", color:"rgba(255,255,255,.3)", fontFamily:"var(--font-body)", padding:30 }}>No expenses logged yet</div>
      )}
    </div>
  );
}
  ...
  return (
    <div style={{ display:"flex", gap:6, flexWrap:"nowrap", overflowX:"auto", padding:"8px 14px", background:"rgba(255,255,255,.02)", borderBottom:"1px solid rgba(255,255,255,.06)", WebkitOverflowScrolling:"touch" }}>
      <button onClick={function(){ onSetSelectedDate(null); }}
        ...
        <span style={{ fontFamily:"var(--font-head)", color:allSel?"#FF6B35":"rgba(255,255,255,.5)", fontSize:12, fontWeight:allSel?700:400 }}>All</span>
      </button>
      {dates.map(function(d) {
        ...
        return (
          <button key={d} onClick={function(){ var dt=parseDate(d); if(dt) onSetSelectedDate(dt.toDateString()); }}
            ...
            <span style={{ fontFamily:"var(--font-head)", color:sel?"#00D4FF":"rgba(255,255,255,.6)", fontSize:12, fontWeight:sel?700:400 }}>{dateLabel(d)}</span>
            <span style={{ background:sel?"rgba(0,212,255,.3)":"rgba(255,255,255,.1)", borderRadius:20, padding:"1px 6px", fontFamily:"var(--font-body)", color:sel?"#00D4FF":"rgba(255,255,255,.4)", fontSize:10 }}>{dateCounts[d]}</span>
          </button>
        );
      })}
    </div>
  ...
  return (
    <div style={{ maxWidth:430, margin:"0 auto", background:"#070C1A", height:"100dvh", display:"flex", flexDirection:"column", position:"relative", width:"100%" }}>
      ...
      {clearConfirm && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.85)", zIndex:9999, display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
          <div style={{ background:"#0F1629", borderRadius:20, padding:28, maxWidth:320, width:"100%", border:"1px solid rgba(239,68,68,.3)" }}>
            <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:18, fontWeight:800, marginBottom:8 }}>Clear Day?</div>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.5)", fontSize:14, marginBottom:24, lineHeight:1.6 }}>
              This will delete ALL orders, expenses and transfers. Driver profiles will be kept.
            </div>
            <div style={{ display:"flex", gap:12 }}>
              <button onClick={onCancelClear} style={{ flex:1, background:"rgba(255,255,255,.08)", border:"none", borderRadius:12, padding:13, color:"#fff", fontFamily:"var(--font-head)", fontWeight:700, fontSize:14, cursor:"pointer" }}>Cancel</button>
              <button onClick={onConfirmClear} style={{ flex:1, background:"linear-gradient(135deg,#EF4444,#DC2626)", border:"none", borderRadius:12, padding:13, color:"#fff", fontFamily:"var(--font-head)", fontWeight:700, fontSize:14, cursor:"pointer" }}>Yes, Clear</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ padding:"16px 20px 12px", background:"#070C1A", borderBottom:"1px solid rgba(255,255,255,.06)", flexShrink:0 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:19, fontWeight:800 }}>Admin <span style={{ color:"#FF6B35" }}>Panel</span></div>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12 }}>AMTEL TELECOM   {new Date().toLocaleDateString("en-KW")}</div>
          {syncing && <span style={{ fontFamily:"var(--font-body)", fontSize:11, color:"#F59E0B", marginLeft:8 }}>Syncing...</span>}
          {!syncing && dbConnected && <span style={{ fontFamily:"var(--font-body)", fontSize:11, color:"#10B981", marginLeft:8 }}>DB Connected</span>}
          {!syncing && !dbConnected && <span style={{ fontFamily:"var(--font-body)", fontSize:11, color:"rgba(255,255,255,.2)", marginLeft:8 }}>Local only</span>}
          </div>
          <button onClick={function(){ if(window.confirm("Clear all COLLECTED orders from driver dashboards? Pending and delivered orders kept.")) onClearCollected(); }}
            style={{ background:"rgba(245,158,11,.12)", border:"1px solid rgba(245,158,11,.25)", borderRadius:8, padding:"5px 10px", color:"#F59E0B", fontFamily:"var(--font-body)", fontSize:11, cursor:"pointer" }}>
            Clear Collected
          </button>
          <button onClick={onLogout} style={{ background:"rgba(239,68,68,.1)", border:"1px solid rgba(239,68,68,.3)", borderRadius:10, padding:"6px 12px", color:"#EF4444", fontFamily:"var(--font-body)", fontSize:12, cursor:"pointer" }}>Sign Out</button>
        </div>
      </div>
        ...
        {DRIVERS.filter(function(d){ return d.status !== "inactive"; }).map(function(d) {
          ...
          return (
            <div key={d.id} style={{ display:"flex", alignItems:"center", gap:6, flexShrink:0, background:"rgba(255,255,255,.04)", borderRadius:20, padding:"3px 10px 3px 6px", border:"1px solid rgba(255,255,255,.06)" }}>
              ...
              <span style={{ fontFamily:"var(--font-body)", color:isOnline?"#10B981":"rgba(255,255,255,.3)", fontSize:11, fontWeight:600 }}>{d.name}</span>
      ...
      <div style={{ display:"flex", background:"#0A1020", borderBottom:"1px solid rgba(255,255,255,.06)", flexShrink:0, position:"sticky", top:0, zIndex:10 }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ flex:1, background:"none", border:"none", padding:"12px 0 8px", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:2, position:"relative" }}>
            ...
            <span style={{ fontFamily:"var(--font-body)", fontSize:10, fontWeight:tab===t.id?600:400, color:tab===t.id?"#FF6B35":"rgba(255,255,255,.35)" }}>{t.label}</span>
            {tab===t.id && <div style={{ width:4, height:4, borderRadius:"50%", background:"#FF6B35" }} />}
            {t.badge > 0 && <div style={{ position:"absolute", top:4, right:"50%", transform:"translateX(100%)", background:"#EF4444", borderRadius:10, padding:"1px 5px", fontSize:9, color:"#fff", fontFamily:"var(--font-body)", fontWeight:700 }}>{t.badge}</div>}
          </button>
        ))}
      </div>
      ...
      <div style={{ flex:1, overflowY:"auto", paddingTop:16, display:"flex", flexDirection:"column", WebkitOverflowScrolling:"touch" }}>
        ...
        {tab==="notifs" && (
          <div style={{ padding:"0 16px 80px" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
              <div>
                <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:15, fontWeight:700 }}> Order Status Alerts</div>
                <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12 }}>Real-time updates from drivers</div>
              </div>
              {adminNotifs.length > 0 && (
                <button onClick={onClearNotifs} style={{ background:"none", border:"none", color:"#FF6B35", fontFamily:"var(--font-body)", fontSize:12, cursor:"pointer" }}>
                  Mark all read
                </button>
              )}
            </div>
              ...
              <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:10 }}>
                {["all","Trikart Online","Webstore Online","ReStore Online"].map(function(s){
                  ...
                  return <button key={s} onClick={function(){ setAlertStoreFilter(s); }}
                    style={{ background:sel?"rgba(0,212,255,.15)":"rgba(255,255,255,.06)", border:sel?"1.5px solid #00D4FF":"1px solid rgba(255,255,255,.1)", borderRadius:20, padding:"4px 12px", color:sel?"#00D4FF":"rgba(255,255,255,.5)", fontFamily:"var(--font-body)", fontSize:11, cursor:"pointer" }}>
                ...
                <div>
                  <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:12 }}>
                    <button onClick={function(){ setFilterDate("all"); }}
                      style={{ background:filterDate==="all"?"rgba(255,107,53,.2)":"rgba(255,255,255,.06)", border:filterDate==="all"?"1px solid #FF6B35":"1px solid rgba(255,255,255,.1)", borderRadius:20, padding:"4px 12px", color:filterDate==="all"?"#FF6B35":"rgba(255,255,255,.5)", fontFamily:"var(--font-body)", fontSize:12, cursor:"pointer" }}>All</button>
                    {dates.map(function(d){
                      return <button key={d} onClick={function(){ setFilterDate(d); }}
                        style={{ background:filterDate===d?"rgba(255,107,53,.2)":"rgba(255,255,255,.06)", border:filterDate===d?"1px solid #FF6B35":"1px solid rgba(255,255,255,.1)", borderRadius:20, padding:"4px 12px", color:filterDate===d?"#FF6B35":"rgba(255,255,255,.5)", fontFamily:"var(--font-body)", fontSize:12, cursor:"pointer" }}>{dateLabel(d)}</button>;
                    })}
                  </div>
                  {filtered.length === 0 ? (
                    <div style={{ textAlign:"center", color:"rgba(255,255,255,.3)", fontFamily:"var(--font-body)", padding:40 }}>No notifications for this day</div>
              ...
              return (
                <div key={n.id} onClick={() => onMarkNotifRead(n.id)}
                  ...
                  <div style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
                    <div style={{ width:36, height:36, borderRadius:10, background:n.notifType==="help"?"rgba(239,68,68,.2)":sc.bg, display:"flex", alignItems:"center", justifyContent:"center", fontSize:n.notifType==="help"?14:18, flexShrink:0, fontFamily:"var(--font-head)", fontWeight:800, color:n.notifType==="help"?"#EF4444":"inherit" }}>{n.notifType==="help"?"SOS":sc.icon}</div>
                    <div style={{ flex:1 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:3 }}>
                        <div style={{ fontFamily:"var(--font-head)", color:n.notifType==="help"?"#EF4444":(n.read?"rgba(255,255,255,.5)":sc.color), fontSize:13, fontWeight:700 }}>
                        ...
                        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.3)", fontSize:11 }}>
                          {smartTime(n.time)}
                        </div>
                      </div>
                      <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.7)", fontSize:13 }}>{n.notifType==="help" ? n.note : n.customer}</div>
                      {n.notifType==="help" ? (
                        <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12, marginTop:2 }}>
                        ...
                        <div>
                          ...
                          {n.onlineOrderNo && (
                            <div style={{ fontFamily:"var(--font-body)", color:"rgba(0,212,255,.7)", fontSize:11, marginTop:2 }}>
                              Online Order No: <span style={{ fontWeight:700, color:"#00D4FF" }}>{n.onlineOrderNo}</span>
                            </div>
                          )}
                        </div>
                      )}
                      {n.notifType!=="help" && <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12 }}> Driver: {n.driver}</div>}
                      {n.note && n.notifType!=="help" && <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:11, marginTop:3, fontStyle:"italic" }}> {n.note}</div>}
                    </div>
        ...
        {tab==="transfers" && (
          <div style={{ padding:"0 16px 80px" }}>
            <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:15, fontWeight:700, marginBottom:4 }}> Transfer Requests</div>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12, marginBottom:16 }}>Review and approve driver transfer requests</div>

            {transfers.length === 0 ? (
              <div style={{ textAlign:"center", color:"rgba(255,255,255,.3)", fontFamily:"var(--font-body)", padding:40 }}>No transfer requests yet</div>
              ...
              return (
                <div key={tr.id} style={{ background:isPending?"rgba(255,255,255,.05)":"rgba(255,255,255,.02)", border:"1px solid " + (isPending?"rgba(255,107,53,.3)":tr.status==="approved"?"rgba(16,185,129,.2)":"rgba(239,68,68,.2)"), borderRadius:16, padding:16, marginBottom:12 }}>
                  {/* Status banner */}
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                    <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11 }}>{new Date(tr.createdAt).toLocaleTimeString("en-KW", { hour:"2-digit", minute:"2-digit" })}</div>
                    <span style={{ background:isPending?"rgba(245,158,11,.15)":tr.status==="approved"?"rgba(16,185,129,.15)":"rgba(239,68,68,.15)", color:isPending?"#F59E0B":tr.status==="approved"?"#10B981":"#EF4444", borderRadius:20, padding:"3px 10px", fontSize:11, fontFamily:"var(--font-body)", fontWeight:600 }}>
                  ...
                  <div style={{ background:"rgba(0,0,0,.2)", borderRadius:10, padding:"10px 12px", marginBottom:10 }}>
                    <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:13, fontWeight:700 }}>{tr.order.customer}</div>
                    <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:11, marginTop:2 }}>#{tr.order.invoiceNo}   {tr.order.store}   {fmt(tr.order.total)}</div>
                  </div>

                  {/* Transfer direction */}
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:tr.reason?8:12 }}>
                    <div style={{ background:"rgba(255,107,53,.15)", borderRadius:8, padding:"5px 10px", fontFamily:"var(--font-head)", color:"#FF6B35", fontSize:12, fontWeight:700 }}>{fromDriver?.name || tr.fromDriverId}</div>
                    <span style={{ color:"rgba(255,255,255,.4)", fontSize:14 }}>-></span>
                    <div style={{ background:"rgba(0,212,255,.15)", borderRadius:8, padding:"5px 10px", fontFamily:"var(--font-head)", color:"#00D4FF", fontSize:12, fontWeight:700 }}>{toDriver?.name || tr.toDriverId}</div>
                  </div>

                  {tr.reason && <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12, marginBottom:12, fontStyle:"italic" }}> "{tr.reason}"</div>}

                  {isPending && (
                    <div style={{ display:"flex", gap:8 }}>
                      <button onClick={() => { onRejectTransfer(tr.id); showToast("Transfer rejected"); }}
                        style={{ flex:1, background:"rgba(239,68,68,.1)", border:"1px solid rgba(239,68,68,.3)", borderRadius:10, padding:"10px", color:"#EF4444", fontFamily:"var(--font-head)", fontWeight:700, fontSize:13, cursor:"pointer" }}>
                        x Reject
                      </button>
                      <button onClick={() => { onApproveTransfer(tr.id); showToast(" Order transferred to " + (toDriver?.name) + "!", "success"); }}
                        style={{ flex:2, background:"linear-gradient(135deg,#10B981,#00D4FF)", border:"none", borderRadius:10, padding:"10px", color:"#fff", fontFamily:"var(--font-head)", fontWeight:700, fontSize:13, cursor:"pointer" }}>
                         Approve Transfer
                      </button>
                    </div>
                  )}
  ...
  return (
    <div style={{ maxWidth:430, margin:"0 auto", background:"#0A0F1E", height:"100dvh", display:"flex", flexDirection:"column", position:"relative", width:"100%" }}>
      ...
      <div style={{ padding:"16px 20px 12px", background:"#0A0F1E", borderBottom:"1px solid rgba(255,255,255,.06)", flexShrink:0 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:18, fontWeight:800 }}>DeliverFlow</div>
            <div style={{ display:"flex", alignItems:"center", gap:8, marginTop:2, flexWrap:"wrap" }}>
              <span style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12 }}>Hi, {user.name}   {myOrders.length} orders today</span>
            </div>
          </div>
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            {pending > 0 && <span style={{ background:"rgba(245,158,11,.2)", color:"#F59E0B", borderRadius:20, padding:"3px 10px", fontFamily:"var(--font-body)", fontSize:12, fontWeight:600 }}> {pending} to collect</span>}
            <button onClick={onLogout} style={{ background:"rgba(255,255,255,.08)", border:"none", borderRadius:8, padding:"5px 10px", color:"rgba(255,255,255,.5)", fontFamily:"var(--font-body)", fontSize:11, cursor:"pointer" }}>Sign Out</button>
          </div>
        </div>
      </div>
      ...
      {myOrders.length === 0 && (
        <div style={{ background:"rgba(245,158,11,.08)", border:"1px solid rgba(245,158,11,.2)", borderRadius:0, padding:"12px 20px", display:"flex", gap:10, alignItems:"center" }}>
          <span style={{ fontSize:18 }}></span>
          <span style={{ fontFamily:"var(--font-body)", color:"#F59E0B", fontSize:13 }}>Waiting for admin to assign orders... Check back soon.</span>
        </div>
      )}
      ...
      <div style={{ display:"flex", background:"#0D1326", borderTop:"1px solid rgba(255,255,255,.06)", flexShrink:0, paddingBottom:"env(safe-area-inset-bottom,0px)" }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ flex:1, background:"none", border:"none", padding:"12px 0 8px", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:2, position:"relative" }}>
            ...
            <span style={{ fontFamily:"var(--font-body)", fontSize:11, fontWeight:tab===t.id?600:400, color:tab===t.id?"#00D4FF":"rgba(255,255,255,.3)" }}>{t.label}</span>
            {tab===t.id && <div style={{ width:4, height:4, borderRadius:"50%", background:"#00D4FF" }} />}
            {t.badge > 0 && <div style={{ position:"absolute", top:6, right:"50%", transform:"translateX(100%)", background:"#10B981", borderRadius:10, padding:"1px 5px", fontSize:9, color:"#fff", fontFamily:"var(--font-body)", fontWeight:700 }}>{t.badge}</div>}
          </button>
        ))}
      </div>
    </div>
    ...
    <div style={{ height:"100dvh", background:"#0A0F1E", display:"flex", flexDirection:"column", maxWidth:480, margin:"0 auto", width:"100%", position:"relative" }}>
      ...
      <div style={{ padding:"16px 20px 12px", background:"#0A0F1E", borderBottom:"1px solid rgba(255,255,255,.06)", flexShrink:0 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:18, fontWeight:800 }}>DeliverFlow</div>
            <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12 }}>{myStore} · Store Admin</div>
          </div>
          <button onClick={onLogout} style={{ background:"rgba(255,255,255,.08)", border:"none", borderRadius:8, padding:"5px 10px", color:"rgba(255,255,255,.5)", fontFamily:"var(--font-body)", fontSize:11, cursor:"pointer" }}>Sign Out</button>
        </div>
            ...
            return (
              <div key={d.id} style={{ display:"flex", alignItems:"center", gap:5, flexShrink:0 }}>
                <div style={{ width:8, height:8, borderRadius:"50%", background:isOnline?"#10B981":"#374151",
                  boxShadow:isOnline?"0 0 6px #10B981":"none",
                  animation:(isOnline&&isActive)?"pulse 1.2s ease-in-out infinite":"none" }} />
                <span style={{ fontFamily:"var(--font-body)", color:isOnline?"#10B981":"rgba(255,255,255,.3)", fontSize:11 }}>{d.name}</span>
              </div>
            );
          })}
        </div>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"16px 16px 40px" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
          <div style={{ fontFamily:"var(--font-head)", color:"#fff", fontSize:16, fontWeight:700 }}>
            {myStore} Updates
            {unread > 0 && <span style={{ background:"#EF4444", color:"#fff", borderRadius:20, padding:"2px 8px", fontSize:11, fontFamily:"var(--font-body)", marginLeft:8 }}>{unread}</span>}
          </div>
          {myNotifs.length > 0 && (
            <button onClick={onClearNotifs} style={{ background:"none", border:"none", color:"#FF6B35", fontFamily:"var(--font-body)", fontSize:12, cursor:"pointer" }}>Mark all read</button>
          )}
        </div>

        {myNotifs.length === 0 ? (
          <div style={{ textAlign:"center", color:"rgba(255,255,255,.3)", fontFamily:"var(--font-body)", padding:40 }}>No updates yet for {myStore}</div>
            ...
            <div key={n.id} onClick={function(){ onMarkNotifRead(n.id); }}
              ...
              <div style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
                <div style={{ width:36, height:36, borderRadius:10, background:isHelp?"rgba(239,68,68,.2)":sc.bg,
                  ...
                  fontFamily:"var(--font-head)", fontWeight:800, color:isHelp?"#EF4444":"inherit" }}>
                  {isHelp ? "SOS" : sc.icon}
                </div>
                <div style={{ flex:1 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:3 }}>
                    <div style={{ fontFamily:"var(--font-head)", color:isHelp?"#EF4444":(n.read?"rgba(255,255,255,.5)":sc.color), fontSize:13, fontWeight:700 }}>
                  ...
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.7)", fontSize:13 }}>{isHelp ? n.note : n.customer}</div>
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12, marginTop:2 }}>
                    {isHelp ? "Order #"+n.orderId+" · Driver: "+n.driver : n.payment+" · KD "+Number(n.amount).toFixed(3)}
                  </div>
                  <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.4)", fontSize:12 }}>Driver: {n.driver}</div>
                  {n.note && !isHelp && <div style={{ fontFamily:"var(--font-body)", color:"rgba(255,255,255,.35)", fontSize:11, marginTop:3, fontStyle:"italic" }}>{n.note}</div>}
                </div>
                {!n.read && <div style={{ width:8, height:8, borderRadius:"50%", background:sc.color, flexShrink:0, marginTop:4, boxShadow:"0 0 6px "+sc.color }} />}
              </div>
            </div>
